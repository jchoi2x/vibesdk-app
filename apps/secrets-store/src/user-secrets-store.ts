...
export class UserSecretsStore extends DurableObject<Env> {
  ...
  private handleStoreSecret(
    ...
    try {
      ...
      const encryptedNameParts = this.parseEncryptedData(
        data.encryptedNameForStorage,
      );
  ...
  private handleListSecrets(ws: WebSocket, data: { requestId: string }): void {
    ...
    try {
      ...
      const responseSecrets = secrets.map((s) => ({
        id: s.id,
        encryptedName:
          this.uint8ArrayToBase64(s.encryptedName) +
          ':' +
          this.uint8ArrayToBase64(s.nameNonce),
        metadata: s.metadata,
  ...
  private handleGetSecret(
    ws: WebSocket,
    data: { requestId: string; secretId: string },
  ): void {
    // Validate session
    const sessionCheck = this.validateAndRefreshSession();
    if (!sessionCheck.valid) {
  ...
  private validateAndRefreshSession():
    ...
    | {
        valid: false;
        error: string;
        errorType: 'vault_locked' | 'session_expired';
      } {
    if (!this.session) {
      return {
        valid: false,
        error: 'Vault is locked',
        errorType: 'vault_locked',
      };
    }
  ...
  private validateStorageLimits(
    ...
    if (
      encryptedValue &&
      encryptedValue.byteLength > STORAGE_LIMITS.MAX_SECRET_VALUE_SIZE
    ) {
      return `Secret value exceeds maximum size of ${STORAGE_LIMITS.MAX_SECRET_VALUE_SIZE / 1024}KB`;
    }
    if (
      encryptedName &&
      encryptedName.byteLength > STORAGE_LIMITS.MAX_SECRET_NAME_LENGTH
    ) {
  ...
  private parseEncryptedData(base64Data: string): {
  ...
  private cleanupExpiredSession(): void {
    if (
      this.session &&
      Date.now() - this.session.lastAccessedAt > SESSION_TIMEOUT_MS
    ) {
      this.clearSession();
    }
  }
  ...
  getVaultConfig(): VaultConfig | null {
    ...
    return {
      ...
      kdfParams: row.kdf_params
        ? JSON.parse(String(row.kdf_params))
        : undefined,
      prfCredentialId: row.prf_credential_id
        ? String(row.prf_credential_id)
        : undefined,
      prfSalt: row.prf_salt
        ? new Uint8Array(row.prf_salt as ArrayBuffer)
        : undefined,
  ...
  setupVault(request: SetupVaultRequest): boolean {
    const existing = this.ctx.storage.sql.exec(
      `SELECT id FROM vault_config WHERE id = 1`,
    );
    ...
    this.ctx.storage.sql.exec(
      ...
      request.encryptedRecoveryCodes
        ? new Uint8Array(request.encryptedRecoveryCodes)
        : null,
      request.recoveryCodesNonce
        ? new Uint8Array(request.recoveryCodesNonce)
        : null,
  ...
  getSecret(secretId: string): EncryptedSecret | null {
    ...
    return this.rowToEncryptedSecret(
      rows[0] as Record<string, SqlStorageValue>,
    );
  }
  ...
  listSecrets(): SecretListItem[] {
    ...
    return result.toArray().map((row) => {
      ...
      return {
        ...
        metadata: r.metadata
          ? (JSON.parse(String(r.metadata)) as SecretMetadata)
          : undefined,
  ...
  updateSecret(
    ...
    if (update.encryptedValue && update.valueNonce) {
      ...
      values.push(
        new Uint8Array(update.encryptedValue),
        new Uint8Array(update.valueNonce),
      );
    }
    ...
    if (update.encryptedName && update.nameNonce) {
      ...
      values.push(
        new Uint8Array(update.encryptedName),
        new Uint8Array(update.nameNonce),
      );
    }
  ...
  private rowToEncryptedSecret(
    ...
    return {
      ...
      metadata: row.metadata
        ? (JSON.parse(String(row.metadata)) as SecretMetadata)
        : undefined,
  ...
  async requestSecret(query: {
    ...
    if (!secretId) {
      const matches = this.listSecrets().filter((s) => {
        if (query.provider && s.metadata?.provider !== query.provider)
          return false;
        if (query.envVarName && s.metadata?.envVarName !== query.envVarName)
          return false;
        return true;
      });
    ...
    try {
      ...
      const value = await this.decryptSecretValue(
