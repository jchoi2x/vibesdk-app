...
export const PreviewIframe = forwardRef<HTMLIFrameElement, PreviewIframeProps>(
  (
    ...
    const testAvailability = useCallback(
      async (url: string): Promise<'sandbox' | 'dispatcher' | null> => {
        try {
    ...
    const loadWithRetry = useCallback(
      async (url: string, attempt: number) => {
        ...
        if (previewType) {
          ...
          console.log(
            `Preview available (${previewType}) at attempt ${attempt + 1}`,
          );
          setLoadState({
            status: 'postload',
            attempt: attempt + 1,
          ...
          const waitTime =
            previewType === 'dispatcher'
              ? POST_LOAD_WAIT_DISPATCHER
              : POST_LOAD_WAIT_SANDBOX;
          console.log(
            `Waiting ${waitTime}ms before showing preview and capturing screenshot (${previewType} app)`,
          );
        ...
        >
          ...
          <div className="text-center p-8 max-w-md">
            ...
            <div className="text-xs text-text-primary/50 mt-2">
              Preview URLs may take a moment to become available after
              deployment
            </div>
          </div>
        </div>
        ...
        <div className="text-center p-8 max-w-md">
          ...
          <p className="text-text-primary/70 text-sm mb-6">
            {loadState.errorMessage ||
              'The preview failed to load after multiple attempts.'}
          </p>
          <div className="space-y-3">
            ...
            <p className="text-xs text-text-primary/60">
              If the issue persists, please describe the problem in chat so I
              can help diagnose and fix it.
            </p>
          </div>
        </div>
      </div>
