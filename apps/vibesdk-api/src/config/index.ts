import {
  type ConfigurableSecuritySettings,
  getConfigurableSecurityDefaults,
...
export async function getGlobalConfigurableSettings(
  env: Env,
  ...
  try {
    ...
    const mergedConfig = deepMerge<GlobalConfigurableSettings>(
      defaultConfig,
      storedConfig,
    );

    logger.info('Loaded configuration with overrides from KV', {
      storedConfig,
      mergedConfig,
    });
...
export async function getUserConfigurableSettings(
  ...
  try {
    // Try to fetch override config from KV
    const storedConfigJson = await env.VibecoderStore.get(
      `user_config:${userId}`,
    );
    ...
    logger.info(
      `Loaded configuration with overrides from KV for user ${userId}`,
      { globalConfig, storedConfig, mergedConfig },
    );
    invocationUserCache.set(userId, mergedConfig);
    return mergedConfig;
  } catch (error) {
    logger.error(
      `Failed to load configuration from KV for user ${userId}, using defaults`,
      error,
    );
    // On error, fallback to default configuration
    return globalConfig;
