...
export async function proxyToAiGateway(
  ...
  if (!env.AI_PROXY_JWT_SECRET) {
    ...
    return new Response(
      JSON.stringify({
        error: {
          message: 'AI Gateway proxy is not enabled for this platform',
  ...
  try {
    ...
    try {
      ...
      if (!payload.appId || typeof payload.appId !== 'string') {
        return new Response(
          JSON.stringify({
            error: {
              message: 'Invalid token: missing appId',
              type: 'invalid_request_error',
            },
          }),
          {
    ...
    if (app.userId !== userId) {
      console.error(
        `[AI Proxy] UserId mismatch: token userId=${userId}, app userId=${app.userId}`,
      );
      return new Response(
        JSON.stringify({
          error: {
            message: 'Token does not match app owner',
            type: 'invalid_request_error',
          },
        }),
        {
    ...
    console.log(
      `[AI Proxy] Authenticated request from app: ${app.title} (${app.id}), user: ${app.userId}`,
    );
    ...
    console.log(
      `[AI Proxy] Forwarding request to model: ${modelName}, baseURL: ${baseURL}`,
    );
