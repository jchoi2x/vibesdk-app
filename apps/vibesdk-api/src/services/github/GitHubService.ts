...
export class GitHubService {
  ...
  static extractRepoInfo(url: string): { owner: string; repo: string } | null {
    try {
      ...
      GitHubService.logger.error('Failed to parse repository URL', {
        url,
        error,
      });
      return null;
   ...
   */
  private static async modifyReadmeForGitHub(
    fs: MemFS,
    ...
    try {
      ...
      if (!content.includes('[cloudflarebutton]')) {
        GitHubService.logger.info(
          'README.md has no [cloudflarebutton] placeholder',
        );
        return;
      }
      ...
      GitHubService.logger.warn(
        'Failed to modify README, continuing without',
        error,
      );
    }
  }
  ...
  private static findLastCommonCommit(
    ...
    for (let i = reversedLocal.length - 1; i >= 0; i--) {
      const localMsg = GitHubService.normalizeCommitMessage(
        reversedLocal[i].commit.message,
      );
      ...
      const matchingRemote = remoteCommits.find((remote) => {
        ...
        return (
          GitHubService.normalizeCommitMessage(remote.commit.message) ===
          localMsg
        );
      });
  ...
  private static async pushViaGitProtocol(
    ...
    try {
      ...
      const gitUrl = repositoryUrl.endsWith('.git')
        ? repositoryUrl
        : `${repositoryUrl}.git`;
  ...
  static async checkRemoteStatus(options: {
    ...
    try {
      ...
      const commonCommit = GitHubService.findLastCommonCommit(
        localCommits,
        remoteCommits,
      );
      const hasCommonCommit =
        commonCommit !== null || remoteCommits.length === 0;

      const localMessages = new Set(
        localCommits.map((c) =>
          GitHubService.normalizeCommitMessage(c.commit.message),
        ),
      );
