...
import { type KVRateLimitConfig } from '@/services/rate-limit/config';
...
export class KVRateLimitStore {
  ...
  static async increment(
    ...
    try {
      const mainBuckets = this.generateBucketKeys(
        key,
        now,
        mainWindow,
        bucketSize,
      ...
      const bucketResults = await Promise.all(
        allBucketKeys.map(async (bucketKey) => {
          ...
          return {
            key: bucketKey,
            count: value ? parseInt(value, 10) || 0 : 0,
          };
        }),
      );
      ...
      const mainCount = mainBuckets.reduce(
        (sum, bucketKey) => sum + (bucketMap.get(bucketKey) || 0),
        0,
      );
      const burstCount = burstBuckets.reduce(
        (sum, bucketKey) => sum + (bucketMap.get(bucketKey) || 0),
        0,
      );
      ...
      const maxTtlSeconds =
        Math.max(config.period, config.burstWindow ?? 60) +
        (config.bucketSize ?? 10);

      await this.incrementBucketWithRetry(
        kv,
        currentBucketKey,
        maxTtlSeconds,
        incrementBy,
      ...
      return {
        success: true,
        remainingLimit: Math.max(0, config.limit - mainCount - incrementBy),
      };
    } catch (error) {
  ...
  private static generateBucketKeys(
    ...
    for (
      let time = Math.floor(windowStart / bucketSizeMs) * bucketSizeMs;
      time <= now;
      time += bucketSizeMs
    ) {
  ...
  private static async incrementBucketWithRetry(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        await kv.put(bucketKey, newCount.toString(), {
