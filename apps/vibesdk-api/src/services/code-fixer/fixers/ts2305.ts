...
export function fixMissingExportedMember(
  ...
  for (const issue of issues) {
    ...
    try {
      ...
      if (!filesResult) {
        logger.warn(
          `Failed to get source and target files for ${issue.filePath}`,
        );
      ...
      logger.info(
        `Found import: ${importInfo.moduleSpecifier}, named: [${importInfo.namedImports.join(', ')}]`,
      );
      logger.info(`Found target file: ${targetFilePath}`);

      // Extract the missing export name from the error message
      const missingExportName = extractMissingExportName(
        issue.message,
        importInfo.namedImports,
      );
      if (!missingExportName) {
        logger.warn(
          `Could not extract missing export name from message: ${issue.message}`,
        );
        unfixableIssues.push(
          createUnfixableIssue(
            issue,
      ...
      if (existingExports.namedExports.includes(missingExportName)) {
        logger.info(
          `Named export ${missingExportName} already exists in ${targetFilePath}, marking as unfixable`,
        );
        unfixableIssues.push(
          createUnfixableIssue(
            issue,
      ...
      if (
        existingExports.defaultExport &&
        existingExports.defaultExport === missingExportName
      ) {
        logger.info(
          `Default export ${missingExportName} already exists in ${targetFilePath}, marking as unfixable`,
        );
        unfixableIssues.push(
          createUnfixableIssue(
            issue,
      ...
      const exportUsage = usageAnalysis.find(
        (usage) => usage.name === missingExportName,
      );

      logger.info(
        `Usage analysis result: ${exportUsage ? exportUsage.type : 'generic'}`,
      );
      ...
      const modifiedTargetAST = addExportToFile(
        ...
        exportUsage
          ? {
              ...
              parameters:
                (exportUsage.parameters as unknown[])?.map((p) => String(p)) ||
                undefined,
            }
          : undefined,
      );
      ...
      unfixableIssues.push(
        handleFixerError(issue, error as Error, 'TS2305Fixer'),
      );
    }
  }
 ...
 */
function extractModuleSpecifierFromMessage(
  errorMessage: string,
): string | null {
 ...
 * Example: "Module './button' has no exported member 'Button'" -> 'Button'
 */
function extractMissingExportName(
  errorMessage: string,
  namedImports: string[],
): string | null {
  ...
  if (usageAnalysis) {
    switch (usageAnalysis.type) {
      case 'jsx-component':
        ...
        exportDeclaration = createComponentExport(
          exportName,
          usageAnalysis.properties || [],
          stubComment,
        );
        break;
      ...
      case 'function-call':
        ...
        exportDeclaration = createFunctionExport(
          exportName,
          usageAnalysis.parameters || [],
          stubComment,
        );
        break;
      ...
      case 'object-access':
        ...
        exportDeclaration = createObjectExport(
          exportName,
          usageAnalysis.properties || [],
          stubComment,
        );
 ...
 */
function createComponentExport(
  exportName: string,
  props: string[],
  stubComment: string,
): t.Statement {
