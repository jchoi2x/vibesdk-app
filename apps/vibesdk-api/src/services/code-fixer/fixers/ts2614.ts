...
import {
  findImportAtLocation,
  getFileAST,
  getFileExports,
} from '@/services/code-fixer/utils/imports';
...
export function fixImportExportTypeMismatch(
  ...
  for (const issue of issues) {
    logger.info(
      `Processing TS2614 issue: ${issue.message} at ${issue.filePath}:${issue.line}`,
    );
    ...
    try {
      ...
      if (!importInfo) {
        logger.warn(
          `No import found at line ${issue.line} in ${issue.filePath}`,
        );
        unfixableIssues.push({
          issueCode: 'TS2614',
          filePath: issue.filePath,
      ...
      if (!targetFile) {
        logger.warn(
          `Target file not found for module: ${importInfo.moduleSpecifier}`,
        );
        unfixableIssues.push({
          issueCode: 'TS2614',
          filePath: issue.filePath,
      ...
      if (!mismatchAnalysis) {
        logger.warn(
          `Could not determine mismatch type for ${importInfo.moduleSpecifier}`,
        );
        unfixableIssues.push({
          issueCode: 'TS2614',
          filePath: issue.filePath,
      ...
      logger.info(
        `Mismatch analysis: ${mismatchAnalysis.type} - ${mismatchAnalysis.description}`,
      );

      // Apply the fix to the import statement
      const fixedAST = fixImportStatement(
        sourceAST,
        importInfo,
        mismatchAnalysis,
      );
      ...
      logger.info(
        `Successfully fixed import/export mismatch in ${issue.filePath}`,
      );
    } catch (error) {
      logger.error(
      ...
      unfixableIssues.push(
        handleFixerError(issue, error as Error, 'TS2614Fixer'),
      );
    }
  }
...
function analyzeMismatch(
  ...
  if (
    importInfo.defaultImport &&
    !targetExports.defaultExport &&
    targetExports.namedExports.length > 0
  ) {
  ...
  if (importInfo.namedImports.length > 0) {
    ...
    const invalidNamedImports = importInfo.namedImports.filter(
      (name) => !targetExports.namedExports.includes(name),
    );
    ...
    if (
      needsDefaultConversion.length > 0 ||
      validNamedImports.length > 0 ||
  ...
  if (importInfo.defaultImport && targetExports.namedExports.length > 0) {
    ...
    const similarNamed = targetExports.namedExports.find((name) => {
      const distance = levenshteinDistance(
        importInfo.defaultImport!.toLowerCase(),
        name.toLowerCase(),
      );
      return distance <= 2;
    });
  ...
  traverseAST(ast, {
    ImportDeclaration(path) {
      if (
        ...
        path.node.source.value === importInfo.moduleSpecifier
      ) {
        switch (mismatchAnalysis.type) {
          case 'named-to-default':
            ...
            if (mismatchAnalysis.targetName) {
              ...
              const localName =
                orig && t.isIdentifier(orig.local)
                  ? orig.local.name
                  : mismatchAnalysis.targetName;
          ...
          case 'complex-partial':
            ...
            if (mismatchAnalysis.additionalData) {
              ...
              const defaultConversions =
                mismatchAnalysis.additionalData.defaultConversions || [];
              if (defaultConversions.length > 0) {
                ...
                const defaultLocal =
                  defaultSpec && t.isIdentifier(defaultSpec.local)
                    ? defaultSpec.local.name
                    : firstDefault;
                newSpecifiers.push(
                  t.importDefaultSpecifier(t.identifier(defaultLocal)),
                );
              }
              ...
              const typoCorrections =
                mismatchAnalysis.additionalData.typoCorrections || [];
              for (const correction of typoCorrections) {
                if (correction) {
                  ...
                  newSpecifiers.push(
                    t.importSpecifier(
                      t.identifier(local),
                      t.identifier(correction.correct),
                    ),
                  );
                }
              }
              ...
              const defaultLocal =
                invalidSpec && t.isIdentifier(invalidSpec.local)
                  ? invalidSpec.local.name
                  : mismatchAnalysis.targetName;
              newSpecifiers.push(
                t.importDefaultSpecifier(t.identifier(defaultLocal)),
              );
