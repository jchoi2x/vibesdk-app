...
import {
  resolvePathAlias,
  findModuleFile,
} from '@/services/code-fixer/utils/paths';
import {
...
export function fixIncorrectNamedImport(
  ...
  for (const [filePath, fileIssues] of issuesByFile) {
    try {
      ...
      for (const issue of fileIssues) {
        ...
        if (!parseResult) {
          ...
          unfixableIssues.push(
            createUnfixableIssue(
              issue,
              'Could not parse error message to extract import names',
            ),
          );
          continue;
        }

        const { moduleSpecifier, incorrectImport, suggestedImport } =
          parseResult;
        ...
        if (isExternalModule(moduleSpecifier)) {
          ...
          unfixableIssues.push(
            createExternalModuleError(issue, moduleSpecifier),
          );
          continue;
        }
        ...
        if (importInfo.moduleSpecifier !== moduleSpecifier) {
          logger.warn(
            `Module specifier mismatch. Expected: ${moduleSpecifier}, Found: ${importInfo.moduleSpecifier}`,
          );
          unfixableIssues.push(
            createUnfixableIssue(
              issue,
              'Module specifier does not match error message',
            ),
          );
          continue;
        }
        ...
        if (!importInfo.namedImports.includes(incorrectImport)) {
          ...
          unfixableIssues.push(
            createUnfixableIssue(
              issue,
              `Named import '${incorrectImport}' not found in import statement`,
            ),
          );
          continue;
        }
        ...
        if (targetFile) {
          ...
          if (targetAST) {
            ...
            if (
              exports.namedExports.includes(suggestedImport) ||
              ...
              replacementsByImport
                .get(key)!
                .set(incorrectImport, suggestedImport);
              processedIssues.push(issue);
            } else {
              logger.warn(
                `Suggested export '${suggestedImport}' not found in ${targetFile}`,
              );
              unfixableIssues.push(
                createUnfixableIssue(
                  issue,
                  `Suggested export '${suggestedImport}' not found in target module`,
                ),
              );
            }
          } else {
            ...
            unfixableIssues.push(
              createUnfixableIssue(issue, 'Could not parse target module file'),
            );
          }
        } else {
          logger.warn(
            `Could not find target module file for: ${moduleSpecifier}`,
          );
          unfixableIssues.push(
            createUnfixableIssue(issue, 'Target module file not found'),
          );
        }
      }
      ...
      if (replacementsByImport.size > 0) {
        const fixedAST = applyMultipleNamedImportReplacements(
          sourceAST,
          replacementsByImport,
        );
        if (fixedAST) {
      ...
      for (const issue of fileIssues) {
        unfixableIssues.push(
          handleFixerError(issue, error as Error, 'TS2724Fixer'),
        );
      }
    }
  }
  ...
  logger.info(
    logs.completed(
      fixedIssues.length,
      unfixableIssues.length,
      modifiedFilesMap.size,
      newFiles.length,
    ),
  );
...
export function parseTS2724ErrorMessage(errorMessage: string): {
  ...
  // "'@/components/ui/sonner' has no exported member named 'toast'. Did you mean 'Toaster'?"
  const pattern1 =
    /^'([^']+)'\s+has no exported member named\s+'([^']+)'\.?\s+Did you mean\s+'([^']+)'\??\s*$/i;
  ...
  // "Module './utils' has no exported member 'utilFunction'. Did you mean 'utilityFunction'?"
  const pattern2 =
    /^Module\s+'([^']+)'\s+has no exported member\s+'([^']+)'\.?\s+Did you mean\s+'([^']+)'\??\s*$/i;
  ...
  // "'react' has no exported member named 'useCallback'. Did you mean 'useCallBack'?"
  const pattern3 =
    /^'([^']+)'\s+has no exported member\s+named\s+'([^']+)'\.?\s+Did you mean\s+'([^']+)'\??\s*$/i;
