  ...
  for (const [filePath, fileIssues] of issuesByFile) {
    try {
      ...
      for (const issue of fileIssues) {
        ...
        const usageContext = analyzeUsageContext(
          fileContent,
          undefinedName,
          issue.line,
        );
      ...
      for (const issue of fileIssues) {
        unfixableIssues.push(
          handleFixerError(issue, error as Error, 'TS2304Fixer'),
        );
      }
    }
  }
...
function analyzeUsageContext(
  ...
  const promisePattern = new RegExp(
    `${name}\\s*\\([^)]*\\)\\s*\\.\\s*(then|catch|finally)`,
    'g',
  );
  if (
    functionPattern.test(errorLine) ||
    asyncPattern.test(contextLines) ||
    promisePattern.test(contextLines)
  ) {
    return 'function';
  }
  ...
  const declarationPattern = new RegExp(
    `\\b(let|const|var)\\s+${name}\\b`,
    'g',
  );
  if (
    assignmentPattern.test(errorLine) &&
    !declarationPattern.test(errorLine)
  ) {
    return 'variable';
  }
  ...
  const valuePattern = new RegExp(
    `(return|if|while|for|switch|case|throw).*\\b${name}\\b`,
    'g',
  );
