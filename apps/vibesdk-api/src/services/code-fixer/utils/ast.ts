...
export function mergeParseOptions(custom?: ParseOptions): ParserOptions {
  ...
  return {
    ...
    plugins: custom.plugins
      ? (custom.plugins as ParserOptions['plugins'])
      : defaults.plugins,
  };
}
...
export function parseCode(code: string, options?: ParseOptions): t.File {
  ...
  try {
    logger.debug(
      `Parsing code (${code.length} characters) with TypeScript plugins`,
    );
...
export function traverseAST(ast: t.Node, visitor: Visitor): void {
  try {
    ...
    (traverse as unknown as (node: t.Node, opts: Visitor) => void)(
      ast,
      visitor,
    );
    ...
    const shouldRetryNoScope =
      ...
      message.includes('setScope');
    if (shouldRetryNoScope) {
      logger.warn(
        `AST traversal failed with scope error, retrying with noScope: true`,
      );
      try {
        (
          traverse as unknown as (
            node: t.Node,
            opts: Visitor & { noScope?: boolean },
          ) => void
        )(ast, { ...(visitor as object), noScope: true } as Visitor & {
          noScope?: boolean;
        });
        ...
        throw new Error(
          `Failed to traverse AST: ${e2 instanceof Error ? e2.message : 'Unknown error'}`,
        );
      }
    }
...
export function generateCode(
  ...
  try {
    ...
    logger.debug(
      `Successfully generated code (${result.code.length} characters)`,
    );
    return result;
  } catch (error) {
 ...
 */
export function shouldUseJSXExtension(
  usageAnalysis: Array<{ type: string }>,
): boolean {
 ...
 */
export function getAppropriateExtension(
  usageAnalysis: Array<{ type: string }>,
): string {
