...
import {
  createFileAST,
  shouldUseJSXExtension,
  generateCode,
  parseCode,
} from '@/services/code-fixer/utils/ast';
import {
  analyzeImportUsage,
  getFileAST,
...
export function generateStubFileAST(
  ...
  if (importInfo.defaultImport && !usedNames.has(importInfo.defaultImport)) {
    statements.push(
      generateGenericExport(importInfo.defaultImport, true, shouldUseJSX),
    );
  }
...
export function generateStubFileContent(
  ...
  const stubComment =
    '// This is a **STUB** file, please properly implement it or fix its usage\n\n';
 ...
 * Preserves exact logic from working implementation
 */
function generateExportForUsage(
  usage: ImportUsage,
  shouldUseJSX: boolean,
): t.Statement | null {
  switch (usage.type) {
    case 'jsx-component':
 ...
 * Preserves exact logic from working implementation
 */
function generateVariableExport(
  usage: ImportUsage,
  shouldUseJSX: boolean,
): t.Statement {
 ...
 */
function generateGenericExport(
  name: string,
  isDefault: boolean,
  shouldUseJSX: boolean,
): t.Statement {
