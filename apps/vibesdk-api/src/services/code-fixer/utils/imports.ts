 ...
 */
export function findImportAtLocation(
  ast: t.File,
  line: number,
): ImportInfo | null {
  ...
  for (const node of body) {
    if (t.isImportDeclaration(node)) {
      const moduleSpecifier = t.isStringLiteral(node.source)
        ? node.source.value
        : '';
    ...
    traverseAST(ast, {
      ...
      AwaitExpression(path) {
        if (
          ...
          t.isImport(path.node.argument.callee)
        ) {
  ...
  if (foundImport) {
    logger.debug(
      `Found import at line ${line}: ${JSON.stringify(foundImport)}`,
    );
  } else {
    logger.debug(
...
export function getAllImports(ast: t.File): ImportInfo[] {
  ...
  traverseAST(ast, {
    ImportDeclaration(path) {
      const moduleSpecifier = t.isStringLiteral(path.node.source)
        ? path.node.source.value
        : '';
      const defaultImport = path.node.specifiers.find((s) =>
        t.isImportDefaultSpecifier(s),
      )?.local.name;
 ...
 * Preserves exact logic from working implementation
 */
export function analyzeNameUsage(
  ast: t.File,
  name: string,
): ImportUsage | null {
...
export function getFileAST(filePath: string, files: FileMap): t.File | null {
  ...
  logger.info(
    `ImportUtils: Attempting to parse AST for ${filePath} (${content.length} characters)`,
  );
  logger.info(
    `ImportUtils: First 200 characters: ${content.substring(0, 200)}`,
  );
  ...
  traverseAST(ast, {
    ImportDeclaration(path) {
      if (
        ...
        const namedImports = path.node.specifiers.filter((s) =>
          t.isImportSpecifier(s),
        );
        const namespaceImport = path.node.specifiers.find((s) =>
        ...
        if (namespaceImport && t.isImportNamespaceSpecifier(namespaceImport)) {
          ...
          if (exports.defaultExport && exports.namedExports.length === 0) {
            ...
            changes.push(
              `Converted namespace import '* as ${namespaceImport.local.name}' to default import`,
            );
            fixed = true;
          } else {
          ...
          if (defaultImport) {
            ...
            if (exports.defaultExport) {
              ...
              const targetNamed =
                exports.namedExports.find(
                  (n) =>
                    n === localName ||
                    n.toLowerCase() === localName.toLowerCase(),
              ...
              if (targetNamed) {
                ...
                changes.push(
                  `Changed default import '${localName}' to named import '${targetNamed}'`,
                );
                fixed = true;
              }
            }
          ...
          for (const namedImport of namedImports) {
            if (
              t.isImportSpecifier(namedImport) &&
              t.isIdentifier(namedImport.imported)
            ) {
              const namedImportName = namedImport.imported.name;
              const localAlias = t.isIdentifier(namedImport.local)
              ...
              if (exports.namedExports.includes(namedImportName)) {
                ...
                if (!newSpecifiers.some((s) => t.isImportDefaultSpecifier(s))) {
                  ...
                  changes.push(
                    `Changed named import '${namedImportName}' to default import`,
                  );
                  fixed = true;
                }
              } else {
                ...
                if (caseInsensitiveMatch) {
                  ...
                  changes.push(
                    `Fixed case mismatch: '${namedImportName}' → '${caseInsensitiveMatch}'`,
                  );
                  fixed = true;
                } else {
                  // Import doesn't exist, skip it with warning
                  changes.push(
                    `Removed invalid import '${namedImportName}' (not found in exports)`,
                  );
                  fixed = true;
                }
              }
