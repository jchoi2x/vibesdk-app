...
export class SandboxSdkClient extends BaseSandboxService {
  ...
  private async writeFilesViaScript(
    ...
    try {
      ...
      // Execute with bash
      const { stdout, stderr } = await session.exec(`bash ${scriptPath}`, {
        timeout: 60000,
      });
      ...
      for (const section of metadataSections) {
        try {
          ...
          const instanceDetails: InstanceDetails = {
            ...
            uptime: Math.floor(
              (Date.now() - new Date(metadata.startTime).getTime()) / 1000,
            ),
    ...
    try {
      ...
      try {
        ...
        if (isReady) {
          ...
          this.logger.warn('Development server may not be fully ready', {
            instanceId,
          });
        }
        ...
        this.logger.info(
          'Continuing with server startup despite readiness check error',
          { instanceId },
        );
      }
  ...
  private async provisionTemplateResources(
    ...
    try {
      ...
      if (!parseResult.hasPlaceholders) {
        this.logger.info('No placeholders found in wrangler configuration', {
          instanceId,
        });
        return {
          success: true,
  ...
  > {
    try {
      ...
      const resourceProvisioningResult = await this.provisionTemplateResources(
        instanceId,
        projectName,
      );
      if (
      ...
      try {
        ...
        if (wranglerConfigFile.success) {
          ...
          this.logger.warn('Could not read wrangler.jsonc for KV storage', {
            instanceId,
          });
        }
      } catch (error) {
      ...
      if (isDev(env) || env.USE_TUNNEL_FOR_PREVIEW) {
        this.logger.info('Starting cloudflared tunnel for local development', {
          instanceId,
        });
  ...
  async writeFiles(
    ...
    try {
      ...
      if (
        ...
        filteredFiles.some(
          (file) =>
            file.filePath.endsWith('.ts') || file.filePath.endsWith('.tsx'),
        )
      ) {
    ...
    try {
      ...
      if (applyFilter) {
        if (redactedFiles) {
          ...
          try {
            const metadata =
              await this.getInstanceMetadata(templateOrInstanceId);
    ...
    try {
      ...
      if (lintResult.status === 'fulfilled') {
        ...
        results.lint.summary = {
          errorCount: lintIssues.filter((issue) => issue.severity === 'error')
            .length,
          warningCount: lintIssues.filter(
            (issue) => issue.severity === 'warning',
          ).length,
          infoCount: lintIssues.filter((issue) => issue.severity === 'info')
            .length,
        };
          ...
          if (findResult.exitCode === 0) {
            ...
            if (modulePaths.length > 0) {
              ...
              if (additionalModules.size > 0) {
                this.logger.info('Found additional worker modules', {
