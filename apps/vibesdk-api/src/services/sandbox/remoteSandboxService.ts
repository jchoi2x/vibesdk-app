...
export class RemoteSandboxServiceClient extends BaseSandboxService {
  ...
  constructor(sandboxId: string) {
    super(sandboxId);
    this.logger.info('RemoteSandboxServiceClient initialized', {
  ...
  private async makeRequest<T extends z.ZodTypeAny>(
    ...
    try {
      ...
      headers.set(
        'Authorization',
        `Bearer ${RemoteSandboxServiceClient.token}`,
      );
      ...
      if (!validation.success) {
        this.logger.error(
          'Failed to validate response from runner service',
      ...
      this.logger.error('Error making request to runner service', error, {
        url,
      });
      return {
        success: false,
  ...
  async getFiles(
    ...
    const queryParams =
      filePaths && filePaths.length > 0
        ...
        : '';
    return this.makeRequest(
      `/instances/${instanceId}/files${queryParams}`,
      'GET',
      GetFilesResponseSchema,
    );
  }
  ...
  async runStaticAnalysisCode(
    ...
    const queryParams = lintFiles?.length
      ? `?files=${lintFiles.join(',')}`
      : '';
    return this.makeRequest(
      `/instances/${instanceId}/analysis${queryParams}`,
      'GET',
      StaticAnalysisResponseSchema,
    );
  }
  ...
  async deployToCloudflareWorkers(
    ...
    if (target === 'user') {
      return {
        success: false,
        message:
          'User-targeted deployments are not available with remote sandbox runner',
  ...
  async pushToGitHub(
    ...
    return this.makeRequest(
      `/instances/${instanceId}/github/push`,
      'POST',
      GitHubPushResponseSchema,
      { request, files },
    );
  ...
  async initialize(): Promise<void> {
    ...
    this.logger.info('Remote sandbox service client initialized', {
      sandboxId: this.sandboxId,
    });
  }
  ...
  async getLogs(
    ...
    return this.makeRequest(
      `/instances/${instanceId}/logs${queryString}`,
      'GET',
    );
  }
...
RemoteSandboxServiceClient.init(
  env.SANDBOX_SERVICE_URL,
  env.SANDBOX_SERVICE_API_KEY,
);
