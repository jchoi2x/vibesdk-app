...
export class ResourceProvisioner {
  ...
  constructor(logger: StructuredLogger) {
    ...
    if (!this.accountId || !this.apiToken) {
      this.logger.error(
        'Missing required environment variables for resource provisioning',
        {
      ...
      throw new Error(
        'CLOUDFLARE_ACCOUNT_ID and CLOUDFLARE_API_TOKEN must be set for resource provisioning',
      );
    }
  ...
  async createKVNamespace(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        this.logger.error(
          `Failed to create KV namespace: HTTP ${response.status}`,
          { errorText },
        );
        return {
          success: false,
      ...
      this.logger.info(
        `Successfully created KV namespace: ${result.result.id}`,
        {
          namespaceName,
  ...
  async createD1Database(
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        this.logger.error(
          `Failed to create D1 database: HTTP ${response.status}`,
          { errorText },
        );
        return {
          success: false,
      ...
      this.logger.info(
        `Successfully created D1 database: ${result.result.uuid}`,
        {
          databaseName,
