...
export class TemplateParser {
  ...
  detectPlaceholders(wranglerContent: string): PlaceholderInfo[] {
    ...
    for (const [placeholder, resourceType] of Object.entries(
      ...
      if (wranglerContent.includes(placeholder)) {
        ...
        try {
          binding = this.extractBindingName(
            wranglerContent,
            placeholder,
            resourceType,
          );
          ...
          this.logger.warn(
            `Could not extract binding name for ${placeholder}:`,
            error,
          );
        }
    ...
    this.logger.info(
      `Detected ${placeholders.length} placeholders in wrangler.jsonc`,
    );
    return placeholders;
  }
  ...
  private extractBindingName(
    ...
    try {
      ...
      this.logger.warn(
        'Could not parse wrangler.jsonc as JSON to extract binding name:',
        error,
      );
    }
