...
export class FileTreeBuilder {
  ...
  static buildFromPaths(
    ...
    const excludeDirs = new Set(
      options?.excludeDirs || this.DEFAULT_EXCLUDED_DIRS,
    );
    const excludeFilePatterns =
      options?.excludeFiles || this.DEFAULT_EXCLUDED_FILES;
    ...
    const filteredPaths = normalizedPaths.filter((normalizedPath) => {
      ...
      if (
        filename &&
        fileExcludeRegexes.some((regex) => regex.test(filename))
      ) {
        return false;
      }
    ...
    return this.buildFromPathsWithTypes(
      Array.from(allPaths),
      fileSet,
      rootPath,
    );
  ...
  static generateFindExclusions(options?: {
    ...
    const dirExclusions = excludeDirs
      .map((dir) => `-name "${dir}"`)
      .join(' -o ');
    const fileExclusions = excludeFiles
  ...
  static buildFromFindOutput(findOutput: string): FileTreeNode | undefined {
    ...
    try {
      ...
      if (sections.length < 2) {
        console.error(
          'Invalid find output format: missing ===DIRS=== separator',
        );
        return undefined;
      }
