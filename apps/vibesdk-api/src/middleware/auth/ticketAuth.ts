...
import type {
  TicketAuthConfig,
  TicketResourceType,
} from '@/middleware/auth/routeAuth';
...
export interface TicketAuthenticatable {
  ...
  consumeWsTicket(
    token: string,
  ): Promise<TicketConsumptionResult | null> | TicketConsumptionResult | null;
}
...
function extractAndValidateTicket(
  ...
  if (resourceType === 'vault') {
    return ticket.startsWith(VAULT_TICKET_PREFIX) && parseVaultTicket(ticket)
      ? ticket
      : null;
  }

  return ticket.startsWith(AGENT_TICKET_PREFIX) &&
    AGENT_TICKET_PATTERN.test(ticket)
    ? ticket
    : null;
}
...
export async function authenticateViaTicket(
  ...
  if (!ticket) {
    logger.warn('Invalid or missing ticket', {
  ...
  if (!resourceId) {
    logger.warn('Could not resolve resource ID', {
      resourceType: config.resourceType,
    });
    return null;
  }
  ...
  try {
    ...
    if (!result) {
      logger.warn('Ticket consumption failed', {
        resourceType: config.resourceType,
        resourceId,
      });
      return null;
    ...
    logger.error('Ticket authentication error', {
      resourceType: config.resourceType,
      resourceId,
      error,
    });
