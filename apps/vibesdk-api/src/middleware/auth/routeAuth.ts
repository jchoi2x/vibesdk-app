...
import {
  authenticateViaTicket,
  hasTicketParam,
} from '@/middleware/auth/ticketAuth';
  ...
  try {
    ...
    if (requirement.level === 'owner-only') {
      ...
      if (requirement.resourceOwnershipCheck) {
        if (params) {
          const isOwner = await requirement.resourceOwnershipCheck(
            user,
            params,
            env,
          );
 ...
 */
export async function enforceAuthRequirement(
  c: Context<AppEnv>,
  ...
  if (
    !user &&
    (requirement.level === 'authenticated' ||
      requirement.level === 'owner-only')
  ) {
    ...
    if (authOptions?.ticketAuth && hasTicketParam(request)) {
      const ticketAuth = await authenticateViaTicket(
        request,
        env,
        authOptions.ticketAuth,
        params,
      );
      if (ticketAuth) {
    ...
    if (!user) {
      ...
      try {
        await RateLimitService.enforceAuthRateLimit(
          c.env,
