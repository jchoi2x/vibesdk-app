...
export class DependencyManagement {
  ...
  static mergeDependencies(
    ...
    if (lastPackageJson) {
      try {
        ...
        const packageDeps = parsedPackageJson.dependencies as Record<
          string,
          string
        >;
        ...
        if (packageDeps) {
          ...
          logger?.info(
            `Adding dependencies from last package.json: ${Object.keys(packageDeps).join(', ')}`,
          );
        }
      } catch (error) {
   ...
   */
  static extractDependenciesFromPackageJson(
    packageJson: string,
  ): Record<string, string> {
    try {
