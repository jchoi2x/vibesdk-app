...
export class FileProcessing {
  ...
  static processGeneratedFileContents(
    ...
    const cleanedContents = FileProcessing.cleanFileContents(
      generatedFile.fileContents,
    );
    ...
    if (generatedFile.format === 'unified_diff') {
      ...
      if (originalContents) {
        logger?.info(
          `Valid file contents found for ${generatedFile.filePath}, applying diff`,
        );
      } else {
        logger?.warn(
          `No valid file contents found for ${generatedFile.filePath}, but diff was generated`,
        );
      }
      ...
      try {
        ...
        logger?.error(
          `Error applying diff to file ${generatedFile.filePath}:`,
          error,
        );
    ...
    logger?.info(
      `Setting file contents to cleaned contents ${generatedFile.filePath}`,
    );
    return cleanedContents;
  }
