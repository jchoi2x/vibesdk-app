...
export class DeploymentManager
  extends BaseAgentService<BaseProjectState>
  implements IDeploymentManager
{
  ...
  async executeSetupCommands(
    ...
    if (invalidCommands.length > 0) {
      logger.warn(
        '[commands] DANGEROUS COMMANDS DETECTED IN BOOTSTRAP - FILTERED OUT',
        {
      ...
      try {
        ...
        if (!status.success || !status.isHealthy) {
          ...
          try {
            ...
            logger.info(
              'Instance redeployed successfully after health check failure',
            );
          } catch (redeployError) {
            logger.error(
              'Failed to redeploy after health check failure:',
              redeployError,
            );
          }
        }
  ...
  async fetchRuntimeErrors(clear: boolean = true): Promise<RuntimeError[]> {
    ...
    if (!sandboxInstanceId) {
      throw new Error(
        'No sandbox instance available for runtime error fetching',
      );
    }
    const logger = this.getLog();
    const client = this.getClient();
    ...
    if (errors.length > 0) {
      logger.info(
        `Found ${errors.length} runtime errors: ${errors.map((e) => e.message).join(', ')}`,
      );
    }
  ...
  async deployToSandbox(
    ...
    try {
      ...
      logger.error(
        'Deployment permanently failed after master timeout:',
        error,
      );
      return null;
    } finally {
  ...
  private async executeDeploymentWithRetry(
    ...
    while (true) {
      attempt++;
      logger.info(`Deployment attempt ${attempt}`, {
        sessionId: this.getSessionId(),
      });
      ...
      try {
        ...
        if (attempt % maxAttemptsBeforeSessionReset === 0) {
          logger.warn(
            `${attempt} consecutive failures, resetting sessionId for fresh sandbox`,
          );
          this.resetSessionId();
        }
        ...
        const backoffMs = Math.min(
          1000 * Math.pow(2, Math.min(attempt - 1, 5)),
          30000,
        );
  ...
  private async deploy(params: DeploymentParams): Promise<DeploymentResult> {
    ...
    const { sandboxInstanceId, previewURL, tunnelURL, redeployed } =
      instanceResult;
    ...
    if (clearLogs) {
      try {
        logger.info('Clearing logs and runtime errors for instance', {
          instanceId: sandboxInstanceId,
        });
        await Promise.all([
  ...
  async ensureInstance(redeploy: boolean): Promise<DeploymentResult> {
    ...
    if (sandboxInstanceId) {
      ...
      if (status.success && status.isHealthy) {
        logger.info(
          `DEPLOYMENT CHECK PASSED: Instance ${sandboxInstanceId} is running`,
        );
        return {
          sandboxInstanceId,
      ...
      logger.error(
        `DEPLOYMENT CHECK FAILED: Failed to get status for instance ${sandboxInstanceId}, redeploying...`,
      );
    }
