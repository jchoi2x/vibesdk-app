...
export class FileManager implements IFileManager {
  ...
  async syncGeneratedFilesMapFromGit(): Promise<void> {
    ...
    try {
      ...
      console.log('[FileManager] Sync complete', {
        ...
        preservedPurposes: Object.values(newMap).filter(
          (f) => oldMap[f.filePath]?.filePurpose,
        ).length,
      });
    } catch (error) {
  ...
  getAllFiles(): FileState[] {
    const state = this.stateManager.getState();
    return FileProcessing.getAllFiles(
      this.getTemplateDetailsFunc(),
      state.generatedFilesMap,
    );
  }
  ...
  async saveGeneratedFile(
    ...
    const results = await this.saveGeneratedFiles(
      [file],
      commitMessage,
      overwrite,
    );
  ...
  recordFileChanges(
    ...
    for (const file of files) {
      if (
        !isFileModifiable(file.filePath, dontTouchFiles).allowed &&
        !overwrite
      ) {
      ...
      const oldFileContents =
        ...
        (this.getFile(file.filePath)?.fileContents || '');
      ...
      if (oldFileContents !== file.fileContents) {
        try {
          ...
          console.error(
            `Failed to generate diff for file ${file.filePath}:`,
            error,
          );
        }
      }
  ...
  async saveGeneratedFiles(
    ...
    try {
      if (commitMessage) {
        const unescapedMessage = commitMessage
          .replace(/\\n/g, '\n')
          .replace(/\\t/g, '\t');
        console.log(
          `[FileManager] Committing ${fileStates.length} files:`,
          unescapedMessage,
        );
      ...
      console.error(
        `[FileManager] Failed to commit files:`,
        error,
        commitMessage,
      );
    }
  ...
  getTemplateFile(filePath: string): FileOutputType | null {
    try {
      ...
      console.error(
        `[FileManager] Failed to get template file:`,
        error,
        filePath,
      );
      return null;
