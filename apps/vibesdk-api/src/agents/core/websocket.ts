  ...
  try {
    ...
    switch (parsedMessage.type) {
      case WebSocketMessageRequests.SESSION_INIT: {
        ...
        logger.info(
          `Received session init message from ${connection.id}, Disable for now`,
        );
        break;
      }
      case WebSocketMessageRequests.GENERATE_ALL:
        ...
        if (agent.getBehavior().isCodeGenerating()) {
          logger.info(
            'Generation already in progress, skipping duplicate request',
          );
      ...
      case WebSocketMessageRequests.PREVIEW:
        ...
        agent
          ...
          .then((deploymentResult) => {
            logger.info(
              `Preview deployed successfully!, deploymentResult:`,
              deploymentResult,
            );
          })
          .catch((error: unknown) => {
      ...
      case WebSocketMessageRequests.STOP_GENERATION: {
        ...
        sendToConnection(
          connection,
          WebSocketMessageResponses.GENERATION_STOPPED,
          {
      ...
      case WebSocketMessageRequests.RESUME_GENERATION:
        ...
        if (!agent.getBehavior().isCodeGenerating()) {
          sendToConnection(
            connection,
            WebSocketMessageResponses.GENERATION_RESUMED,
            {
      ...
      case WebSocketMessageRequests.GITHUB_EXPORT:
        ...
        sendToConnection(
          connection,
          WebSocketMessageResponses.GITHUB_EXPORT_ERROR,
          {
            message: 'GitHub export via WebSocket is deprecated',
            error:
              'Please use the GitHub export button which will redirect you to authorize with GitHub OAuth',
          },
        );
        break;
      case WebSocketMessageRequests.USER_SUGGESTION:
        ...
        agent
          .handleUserInput(parsedMessage.message, parsedMessage.images)
          .catch((error: unknown) => {
      ...
      case WebSocketMessageRequests.GET_MODEL_CONFIGS:
        ...
        agent
          ...
          .then((configsInfo) => {
            sendToConnection(
              connection,
              WebSocketMessageResponses.MODEL_CONFIGS_INFO,
              {
      ...
      case WebSocketMessageRequests.GET_CONVERSATION_STATE:
        try {
          ...
          sendToConnection(
            connection,
            WebSocketMessageResponses.CONVERSATION_STATE,
            {
              state,
...
export function sendToConnection<T extends WebSocketMessageType>(
  ...
  try {
    ...
    console.error(
      `Error sending message to connection ${connection.url}:`,
      error,
    );
  }
}
