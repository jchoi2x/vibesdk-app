import {
  type PhaseConceptGenerationSchemaType,
...
import {
  GenerationContext,
  type PhasicGenerationContext,
} from '@/agents/domain/values/GenerationContext';
...
import {
  BaseCodingBehavior,
  type BaseCodingOperations,
} from '@/agents/core/behaviors/base';
  ...
  implements ICodingAgent
{
  ...
  async initialize(
    ...
    if (!templateInfo || !templateInfo.templateDetails) {
      throw new Error(
        'Phasic initialization requires templateInfo.templateDetails',
      );
    }
    const {
      query,
    ...
    this.logger.info(
      `Using language: ${language}, frameworks: ${frameworks ? frameworks.join(', ') : 'none'}`,
    );
  ...
  migrateStateIfNeeded(): void {
    const migratedState = StateMigration.migratePhasic(
      this.state,
      this.logger,
    ) as PhasicState | null;
    if (migratedState) {
    ...
    if (oldPackageJson) {
      const packageJson = customizePackageJson(
        oldPackageJson,
  ...
  private markPhaseComplete(phaseName: string) {
    ...
    if (!phases.some((p) => p.name === phaseName)) {
      this.logger.warn(
        `Phase ${phaseName} not found in generatedPhases array, skipping save`,
      );
      return;
    }
  ...
  private async launchStateMachine() {
    ...
    const incompletedPhases = generatedPhases.filter(
    ...
    try {
      ...
      while (currentDevState !== CurrentDevState.IDLE) {
        this.logger.info(
          `[generateAllFiles] Executing state: ${currentDevState}`,
        );
        switch (currentDevState) {
          ...
          case CurrentDevState.PHASE_IMPLEMENTING:
            executionResults = await this.executePhaseImplementation(
              phaseConcept,
              userContext,
            );
            currentDevState = executionResults.currentDevState;
    ...
    try {
      ...
      if (
        ...
        if (userContext?.images && userContext.images.length > 0) {
          this.logger.info(
            'Clearing stored user images after passing to phase generation',
          );
          this.pendingUserImages = [];
        }
      }

      const nextPhase = await this.generateNextPhase(
        currentIssues,
        userContext,
        isFinal,
      );
      ...
      if (!nextPhase) {
        this.logger.info(
          'No more phases to implement, transitioning to FINALIZING',
        );
        return {
  ...
  async executePhaseImplementation(
    ...
    try {
      ...
      if (phaseConcept === undefined) {
        ...
        if (phaseConcept === undefined) {
          this.logger.error(
            'No phase concept provided to implement, will call phase generation',
          );
          const results = await this.executePhaseGeneration();
          phaseConcept = results.result;
          if (phaseConcept === undefined) {
            this.logger.error(
              'No phase concept provided to implement, will return',
            );
      ...
      this.logger.info(
        `Phase ${phaseConcept.name} completed, generating next phase`,
      );
  ...
  async executeReviewCycle(): Promise<CurrentDevState> {
    ...
    if (
      issues.runtimeErrors.length > 0 ||
      issues.staticAnalysis.typecheck.issues.length > 0
    ) {
      this.logger.info(
        'Reviewing stage - issues found, prompting user to review and cleanup',
      );
  ...
  async executeFinalizing(): Promise<CurrentDevState> {
    ...
    const { result: phaseConcept, userContext } =
      await this.executePhaseGeneration(true);
    if (!phaseConcept) {
  ...
  async implementPhase(
    ...
    const result = await this.operations.implementPhase.execute(
      {
        ...
        issues,
        isFirstPhase:
          this.state.generatedPhases.filter((p) => p.completed).length === 0,
        ...
        fileChunkGeneratedCallback: streamChunks
          ...
          : (
              _filePath: string,
              _chunk: string,
              _format: 'full_content' | 'unified_diff',
            ) => {},
    ...
    if (result.commands && result.commands.length > 0) {
      this.logger.info(
        'Phase implementation suggested install commands:',
        result.commands,
      );
  ...
  private async applyFastSmartCodeFixes(): Promise<void> {
    try {
      ...
      if (
        staticAnalysis.typecheck.issues.length +
          staticAnalysis.lint.issues.length ==
        0
      ) {
        this.logger.info('No issues found, skipping fast smart code fixes');
        return;
      }
      const issues = staticAnalysis.typecheck.issues.concat(
        staticAnalysis.lint.issues,
      );
