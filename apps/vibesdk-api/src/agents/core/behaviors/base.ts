...
import {
  ...
  type RenderToolCall,
} from '@/agents/operations/UserConversationProcessor';
  ...
  implements ICodingAgent
{
  ...
  protected deepDebugPromise: Promise<
  ...
  async ensureTemplateDetails() {
    ...
    if (!this.templateDetailsCache) {
      if (this.state.templateName === 'scratch') {
        this.logger.info(
          'Skipping template details fetch for scratch baseline',
        );
        return;
      }
      this.logger.info(
        `Loading template details for: ${this.state.templateName}`,
      );
      const results = await BaseSandboxService.getTemplateDetails(
   ...
   */
  private async updateBootstrapScript(
  ...
  async generateReadme() {
    ...
    const readme = await this.operations.simpleGenerateFiles.generateReadme(
    ...
    const debugPromise = (async () => {
      try {
        const previousTranscript =
          this.state.lastDeepDebugTranscript ?? undefined;
        ...
        return {
          ...
          error: `Deep debugger failed: ${String(e)}`,
        };
      } finally {
  ...
  async runStaticAnalysisCode(
    ...
    try {
      ...
      if (templateDetails?.renderMode === 'browser') {
        ...
        analysisResponse =
          await this.deploymentManager.runStaticAnalysis(files);
      }
   ...
   */
  private async runInMemoryAnalysis(
    filePaths?: string[],
  ): Promise<StaticAnalysisResponse> {
   ...
   */
  protected async applyDeterministicCodeFixes(): Promise<
    StaticAnalysisResponse | undefined
  > {
    try {
      ...
      if (staticAnalysis.typecheck.issues.length == 0) {
        this.logger.info(
          'No typecheck issues found, skipping deterministic fixes',
        );
      ...
      this.broadcast(
        WebSocketMessageResponses.DETERMINISTIC_CODE_FIX_COMPLETED,
        {
      ...
      if (fixResult) {
        ...
        if (fixResult.unfixableIssues.length > 0) {
          ...
          const moduleNames = modulesNotFound
            .flatMap((issue) => {
              const match = issue.reason.match(
                /External package ["'](.+?)["']/,
              );
              const name = match?.[1];
        ...
        if (fixResult.modifiedFiles.length > 0) {
          ...
          const fixedFiles = fixResult.modifiedFiles.map((file) => ({
            filePath: file.filePath,
            filePurpose:
              allFiles.find((f) => f.filePath === file.filePath)?.filePurpose ||
              '',
            fileContents: file.fileContents,
          }));
          await this.fileManager.saveGeneratedFiles(
            fixedFiles,
            'fix: applied deterministic fixes',
          );
      ...
      this.logger.info(
        `Applied deterministic code fixes: ${JSON.stringify(fixResult, null, 2)}`,
      );
    } catch (error) {
  ...
  async fetchAllIssues(resetIssues: boolean = false): Promise<AllIssues> {
    ...
    if (isBrowserOnly) {
      const staticAnalysis = await this.runStaticAnalysisCode();
      this.logger.info(
        'Fetched issues (browser-rendered):',
        JSON.stringify({ runtimeErrors: [], staticAnalysis }),
      );
  ...
  async updateProjectName(newName: string): Promise<boolean> {
    try {
      ...
      if (this.state.sandboxInstanceId) {
        try {
          ok = await this.getSandboxServiceClient().updateProjectName(
      ...
      try {
        const appService = new AppService(this.env);
        const dbOk = await appService.updateApp(this.getAgentId(), {
          title: newName,
        });
        ok = ok && dbOk;
      } catch (error) {
  ...
  async execCommands(
    ...
    const result = await this.getSandboxServiceClient().executeCommands(
      sandboxInstanceId,
      commands,
      timeout,
    );
  ...
  updateSlideManifest(file: FileOutputType) {
    ...
    if (this.projectType === 'presentation') {
      ...
      const slidesDirectory =
        templateDetails.slideDirectory ?? '/public/slides';
      if (
        file.filePath.startsWith(slidesDirectory) &&
        ...
        const relativeSlidePath = file.filePath.replace(
          slidesDirectory + '/',
          '',
        );
  ...
  async regenerateFileByPath(
    ...
    if (
      templateDetails &&
      templateDetails.dontTouchFiles &&
      templateDetails.dontTouchFiles.includes(path)
    ) {
      return {
        path,
    ...
    try {
      ...
      if (fmFile) {
        ...
        const f = resp.success
          ? resp.files.find((f) => f.filePath === path)
          : undefined;
    ...
    const regenerated = await this.regenerateFile(
      { filePath: path, fileContents, filePurpose },
      issues,
      0,
    );
    // Invalidate cache
  ...
  async deployPreview(
    ...
    const response = await this.deployToSandbox(
      [],
      forceRedeploy,
      undefined,
      clearLogs,
  ...
  protected getBootstrapCommands() {
    ...
    const { validCommands } =
      validateAndCleanBootstrapCommands(bootstrapCommands);
  ...
  protected async saveExecutedCommands(commands: string[]) {
    ...
    if (hasDependencyCommands) {
      this.logger.info(
        'Dependency commands executed, syncing package.json from sandbox',
      );
      await this.syncPackageJsonFromSandbox();
    }
  }
  ...
  protected async executeCommands(
      ...
      while (currentChunk.length > 0 && retryCount < maxRetries) {
        try {
          ...
          if (!resp.results || !resp.success) {
            ...
            const status =
              await this.getSandboxServiceClient().getInstanceStatus(
                state.sandboxInstanceId,
              );
            if (!status.success || !status.isHealthy) {
              this.logger.error(
                `Instance ${state.sandboxInstanceId} is no longer running`,
              );
              return;
            }
            break;
          ...
          this.logger.warn(
            `${failures.length} commands failed: ${failedCommands.join(', ')}`,
          );
          ...
          if (failedInstallCommands.length > 0 && retryCount < maxRetries) {
            ...
            if (newCommands?.commands && newCommands.commands.length > 0) {
              this.logger.info(
                `AI suggested ${newCommands.commands.length} alternative commands`,
              );
              this.broadcast(WebSocketMessageResponses.COMMAND_EXECUTING, {
    ...
    const failedCommands = commands.filter(
      (cmd) => !successfulCommands.includes(cmd),
    );

    if (failedCommands.length > 0) {
      this.broadcastError(
        'Failed to execute commands',
        new Error(failedCommands.join(', ')),
      );
    } else {
      this.logger.info(
        `All commands executed successfully: ${successfulCommands.join(', ')}`,
      );
    }
  ...
  protected async syncPackageJsonFromSandbox(): Promise<void> {
    try {
      ...
      if (!results || !results.files || results.files.length === 0) {
        this.logger.warn('Failed to fetch package.json from sandbox', {
          results,
        });
        return;
      }
      ...
      const { updated, packageJson } = updatePackageJson(
  ...
  async deleteFiles(
    ...
    try {
      ...
      this.logger.info(
        `Deleted ${filePaths.length} files: ${filePaths.join(', ')}`,
      );
      return { success: true };
    } catch (error) {
  ...
  async handleUserInput(
    ...
    try {
      ...
      const conversationalResponse =
        await this.operations.processUserMessage.execute(
          {
            userMessage,
            conversationState,
      ...
      const { conversationResponse, conversationState: newConversationState } =
  ...
  public async captureScreenshot(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt > 0) {
          this.logger.info(
            `Screenshot retry attempt ${attempt}/${maxRetries}`,
            {
              url,
        ...
        // Capture screenshot
        const base64Screenshot = await this.executeScreenshotCapture(
          url,
          viewport,
        );
        ...
        if (blankDetection.isBlank) {
          ...
          this.logger.warn(
            `Blank screenshot detected on attempt ${attempt + 1}`,
            {
          ...
          if (attempt < maxRetries) {
            const delay =
              SCREENSHOT_CONFIG.RETRY_DELAY_BASE * Math.pow(2, attempt);
          ...
          this.logger.warn(
            'All retry attempts resulted in blank screenshot, using last capture',
          );
        }

        // Process and store the screenshot
        return await this.processAndStoreScreenshot(
          base64Screenshot,
          url,
          viewport,
        );
        ...
        this.logger.error(
          `Screenshot capture attempt ${attempt + 1} failed:`,
          error,
        );
    ...
    // All attempts failed
    const errorMessage =
      lastError?.message || lastBlankReason || 'Unknown error after retries';
    this.broadcast(WebSocketMessageResponses.SCREENSHOT_CAPTURE_ERROR, {
  ...
  private async executeScreenshotCapture(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Browser Rendering API failed: ${response.status} - ${errorText}`,
      );
    }
    ...
    if (!result.success || !result.result.screenshot) {
      throw new Error(
        'Browser Rendering API succeeded but no screenshot returned',
      );
    }
  ...
  private async processAndStoreScreenshot(
    ...
    const signedUrl = await security.signUrl(
      uploadedImage.publicUrl,
      this.getAgentId(),
    );
