import { Agent, type AgentContext, type ConnectionContext } from 'agents';
import {
  type AgentInitArgs,
  type AgentSummary,
  type DeployOptions,
...
import { type BaseCodingBehavior } from '@/agents/core/behaviors/base';
...
import { FileManager } from '@/agents/services/implementations/FileManager';
import { DeploymentManager } from '@/agents/services/implementations/DeploymentManager';
import { GitVersionControl } from '@/agents/git';
import { StateManager } from '@/agents/services/implementations/StateManager';
import { PhasicCodingBehavior } from '@/agents/core/behaviors/phasic';
import { AgenticCodingBehavior } from '@/agents/core/behaviors/agentic';
import { type SqlExecutor } from '@/agents/git';
  ...
  implements AgentInfrastructure<AgentState>
{
  ...
  constructor(ctx: AgentContext, env: Env) {
    ...
    void this
      .sql`CREATE TABLE IF NOT EXISTS full_conversations (id TEXT PRIMARY KEY, messages TEXT)`;
    void this
      .sql`CREATE TABLE IF NOT EXISTS compact_conversations (id TEXT PRIMARY KEY, messages TEXT)`;
  ...
  private createObjective(
    projectType: ProjectType,
  ): ProjectObjective<BaseProjectState> {
  ...
  async onStart(props?: Record<string, unknown> | undefined): Promise<void> {
    ...
    if (readOnlyMode) {
      this.logger().info(
        `Agent ${this.getAgentId()} starting in READ-ONLY mode - skipping expensive initialization`,
      );
      return;
    }
    ...
    this.logger().info(
      `Agent ${this.getAgentId()} session: ${this.state.sessionId} onStart processed successfully`,
    );

    // Load the latest user configs
    const modelConfigService = new ModelConfigService(this.env);
    const userConfigsRecord = await modelConfigService.getUserModelConfigs(
  ...
  async getDecryptedSecret(query: {
    ...
    try {
      ...
      this.logger().info('Secret request failed', {
        query,
        error: String(error),
      });
      return null;
  ...
  deployToSandbox(
    ...
    return this.behavior.deployToSandbox(
      files,
      redeploy,
      commitMessage,
      clearLogs,
  ...
  importTemplate(
    templateName: string,
  ): Promise<{ templateName: string; filesImported: number }> {
  ...
  getConversationState(
    id: string = DEFAULT_CONVERSATION_SESSION_ID,
  ): ConversationState {
    const rows = this.sql<{
      messages: string;
      id: string;
    }>`SELECT * FROM full_conversations WHERE id = ${id}`;
  ...
  addConversationMessage(message: ConversationMessage) {
    ...
    if (
      ...
      conversationState.runningHistory = conversationState.runningHistory.map(
        (msg) => {
    ...
    if (
      ...
      conversationState.fullHistory = conversationState.fullHistory.map(
        (msg) => {
          if (msg.conversationId === message.conversationId) {
            return message;
          }
  ...
  public clearConversation(): void {
    try {
      ...
      void this
        .sql`DELETE FROM full_conversations WHERE id = ${DEFAULT_CONVERSATION_SESSION_ID}`;
      void this
        .sql`DELETE FROM compact_conversations WHERE id = ${DEFAULT_CONVERSATION_SESSION_ID}`;
    ...
    try {
      ...
      if (!head) {
        ...
        if (generatedFiles.length === 0) {
          this.logger().info(
            'No generated files found, skipping initial commit',
          );
          return;
        }
  ...
  async handleBrowserFileServing(request: Request): Promise<Response> {
    ...
    if (!subdomain.startsWith('b-')) {
      this.logger().warn(
        '[BROWSER SERVING] Invalid hostname pattern - missing b- prefix',
        { hostname: url.hostname },
      );
    ...
    this.logger().info('[BROWSER SERVING] Extracted', {
      providedToken,
      filePath,
    });
  ...
  consumeWsTicket(token: string): TicketConsumptionResult | null {
    ...
    if (result) {
      this.logger().info('Ticket consumed successfully', {
