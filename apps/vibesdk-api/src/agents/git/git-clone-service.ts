...
export class GitCloneService {
  ...
  static async buildRepository(
    ...
    try {
          ...
          await git.commit({
            ...
            author: {
              ...
              timestamp: appCreatedAt
                ? Math.floor(appCreatedAt.getTime() / 1000)
                : 0,
            },
          });
        }
      ...
      if (
        templateDetails &&
        templateDetails.allFiles &&
        Object.keys(templateDetails.allFiles).length > 0
      ) {
      ...
      const headContent = headFile
        ? new TextDecoder().decode(headFile.data).trim()
        : null;
      ...
      if (headContent && headContent.startsWith('ref: ')) {
        ...
        const refFile = gitObjects.find(
          (obj) => obj.path === `.git/${refPath}`,
        );
        agentHeadOid = refFile
          ? new TextDecoder().decode(refFile.data).trim()
          : null;
      ...
      for (const obj of gitObjects) {
        if (
          obj.path.startsWith('.git/refs/') ||
          obj.path === '.git/HEAD' ||
          obj.path === '.git/packed-refs'
        ) {
          continue;
        }
      ...
      logger.info(
        'Repository built with proper template base and agent commits',
      );
      return fs;
    } catch (error) {
  ...
  static async handleInfoRefs(fs: MemFS): Promise<string> {
    try {
      ...
      logger.info('Generated info/refs response', {
  ...
  static async handleUploadPack(fs: MemFS): Promise<Uint8Array> {
    try {
      ...
      const nakPacket = new Uint8Array([
        0x30, 0x30, 0x30, 0x38, 0x4e, 0x41, 0x4b, 0x0a,
      ]);
