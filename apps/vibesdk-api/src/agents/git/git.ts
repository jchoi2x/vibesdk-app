...
export class GitVersionControl {
  ...
  async getAllFilesFromHead(): Promise<
  ...
  async stage(files: FileSnapshot[]): Promise<void> {
    ...
    for (const file of files) {
      try {
        ...
        throw new Error(
          `Failed to stage file ${file}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }

    console.log(
      `[Git] Staged ${files.length} files: ${files.map((f) => f.filePath).join(', ')}`,
    );
  }
