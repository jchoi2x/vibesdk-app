 ...
 */
function extractIndentationPattern(text: string): {
  pattern: string;
  lines: string[];
} {
  ...
  const indentedLines = lines.map((line) => {
    ...
    return match
      ? { indent: match[1], content: match[2] }
      : { indent: '', content: line };
  });
...
function findWhitespaceInsensitiveMatch(
  ...
  const actualMatch = findActualMatchBoundaries(
    content,
    searchText,
    normalizedContent,
    normalizedSearch,
...
function findIndentationPreservingMatch(
  content: string,
  searchText: string,
): MatchResult {
  ...
  const actualMatch = findPatternMatchBoundaries(
    content,
    searchText,
    contentPattern,
    searchPattern,
  ...
  return {
    ...
    startIndex: actualMatch.start,
    endIndex: actualMatch.end,
    matchedText: actualMatch.found
      ? content.slice(actualMatch.start, actualMatch.end)
      : '',
    strategy: MatchingStrategy.INDENTATION_PRESERVING,
  };
}
...
function findFuzzyMatch(
  ...
  for (let i = 0; i <= contentLines.length - searchLines.length; i++) {
    ...
    if (similarity >= adjustedThreshold) {
      ...
      const contextScore = validateMatchContext(
        content,
        candidate,
        i,
...
function findActualMatchBoundaries(
  ...
  return {
    start: _normalizedIndex,
    end: _normalizedIndex + normalizedSearch.length,
  };
}
...
function applySearchReplaceBlock(
  ...
  for (const strategy of strategies) {
    ...
    if (strategy === MatchingStrategy.EXACT) {
      ...
      const potentialMatches = countSimilarMatches(
        content,
        block.search,
        strategy,
...
function analyzeSearchBlockQuality(
  ...
  for (const pattern of commonPatterns) {
    if (searchText.includes(pattern)) {
      try {
        ...
        const occurrences = (
          targetContent.match(new RegExp(escapedPattern, 'g')) || []
        ).length;
        if (occurrences > 3) {
    ...
    if (searchText.includes(pattern)) {
      ...
      try {
        ...
        if (occurrences > 2) {
          ...
          specificity *= 0.8;
          warnings.push(
            `Mathematical pattern "${pattern}" may cause ambiguity (${occurrences} occurrences)`,
          );
        }
      } catch (e) {
        // Skip regex errors
  ...
  if (hasCaseStatement) {
    try {
      const caseCount = (
        targetContent.match(/case\s+['"`][^'"`]*['"`]:/g) || []
      ).length;
      if (caseCount > 2) {
        uniquenessScore *= 0.6;
        warnings.push(
          'Switch case detected with multiple similar cases - high ambiguity risk',
        );
      }
    } catch (e) {
      // Skip regex errors
...
function validateMatchContext(
  ...
  const indentations = matchLines.map(
    ...
    for (const block of blocks) {
      ...
      if (blockResult !== null) {
        ...
        if (mergedOptions.enableTelemetry && strategy) {
          result.results.warnings.push(
            `Block at line ${block.lineNumber}: Applied using ${strategy} matching`,
          );
        }
      } else {
...
export function validateDiff(
  ...
  for (const block of blocks) {
    ...
    if (occurrences === 0) {
      errors.push(
        `Block at line ${block.lineNumber}: Search pattern not found`,
      );
    } else if (occurrences > 1) {
      errors.push(
        `Block at line ${block.lineNumber}: Search pattern found ${occurrences} times (ambiguous)`,
      );
    }
  }
  ...
  for (const test of tests) {
    try {
      ...
      if (blocksMatch && errorsMatch) {
        passed++;
        results.push(
          `✅ ${test.name}: PASSED (${blocks.length} blocks, ${errors.length} errors)`,
        );
      } else {
        failed++;
        results.push(`❌ ${test.name}: FAILED`);
        results.push(
          `   Expected: ${test.expectedBlocks} blocks, ${test.expectedErrors} errors`,
        );
        results.push(
          `   Actual: ${blocks.length} blocks, ${errors.length} errors`,
        );
        if (errors.length > 0) {
