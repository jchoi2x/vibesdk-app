...
>>>>>>> REPLACE`;
      ...
      expect(() => applyDiff(original, diff, { strict: true })).toThrow(
        'found 2 times (ambiguous)',
      );
    });
...
const x = 2;`; // Missing end marker
      ...
      expect(() => applyDiff(original, diff, { strict: true })).toThrow(
        'REPLACE block ended prematurely',
      );
    });
...
>>>>>>> REPLACE`;
      ...
      expect(() => applyDiff(original, diff, { strict: true })).toThrow(
        'SEARCH block without corresponding REPLACE',
      );
    });
...
>>>>>>> REPLACE`;
      ...
      expect(
        result.results.errors.some((error) =>
          error.includes('All search/replace blocks failed'),
        ),
      ).toBe(true);
    });
...
>>>>>>> REPLACE`;
      ...
      expect(
        result.results.errors.some((error) =>
          error.includes('Search block not found'),
        ),
      ).toBe(true);
    });
...
>>>>>>> REPLACE`;
      ...
      expect(
        result.results.errors.some((error) =>
          error.includes('Malformed block with multiple separators'),
        ),
      ).toBe(true);
...
another search`;
      ...
      expect(
        result.results.errors.some((error) =>
          error.includes('ended prematurely'),
        ),
      ).toBe(true);
      expect(
        result.results.errors.some((error) =>
          error.includes('end of file reached'),
        ),
      ).toBe(true);
    });
  });
