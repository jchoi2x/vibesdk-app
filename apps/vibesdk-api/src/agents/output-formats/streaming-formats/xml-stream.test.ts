...
import {
  XmlStreamFormat,
  XmlStreamingCallbacks,
} from '@/agents/output-formats/streaming-formats/xml-stream';
...
describe('XmlStreamFormat', () => {
  ...
  describe('Basic XML Parsing', () => {
    ...
    test('should parse multiple elements', () => {
      ...
      expect(elements.get('user_response')![0].content.trim()).toBe(
        'Response here',
      );
      expect(elements.get('enhanced_user_request')![0].content.trim()).toBe(
        'Enhanced request',
      );
    });
  ...
  describe('Streaming Functionality', () => {
    test('should stream element content in real-time', () => {
      ...
      state = parser.parseXmlStream(
        '<user_response>Hello',
        state,
        mockCallbacks,
        config,
      );
      expect(mockCallbacks.onElementContent).toHaveBeenCalledWith(
        'user_response',
        'Hello',
        false,
      );
  ...
  describe('Error Handling and Fallback', () => {
    ...
    test('should use fallback extraction for malformed content', () => {
      const malformedXml =
        '<user_response>Good content</user_response><enhanced_user_request>Also good';
      const config = {
        targetElements: ['user_response', 'enhanced_user_request'],
      };
      let state = parser.initializeXmlState(config);
      ...
      expect(elements.get('enhanced_user_request')![0].content).toBe(
        'Also good',
      );
    });
  });
  ...
  describe('Nested Elements', () => {
    ...
    test('should handle deeply nested elements', () => {
      ...
      expect(level1.children[0].children[0].content.trim()).toBe(
        'Deep content',
      );
    });
  });
  ...
  describe('Real-world LLM Response Scenarios', () => {
    test('should handle typical conversation response format', () => {
      ...
      expect(userResponse.content.trim()).toContain(
    ...
    test('should handle streaming chunks that break mid-word', () => {
      ...
      state = parser.parseXmlStream(
        '<user_response>The quick bro',
        state,
        mockCallbacks,
        config,
      );
      state = parser.parseXmlStream(
        'wn fox jumps over the',
        state,
        mockCallbacks,
        config,
      );
      state = parser.parseXmlStream(
        ' lazy dog</user_response>',
        state,
        mockCallbacks,
        config,
      );
      ...
      const elements = parser.finalizeXmlParsing(state);
      expect(elements.get('user_response')![0].content).toBe(
    ...
    test('should handle LLM responses with extra whitespace and formatting', () => {
      ...
      expect(elements.get('user_response')![0].content).toContain(
        'This is a response',
      );
    });
    ...
    test('should handle incomplete LLM responses gracefully', () => {
      const incompleteXml =
        '<user_response>This response was cut off mid-sente';
      const config = { targetElements: ['user_response'] };
      let state = parser.initializeXmlState(config);
  ...
  describe('Edge Cases and Stress Tests', () => {
    ...
    test('should handle elements with special characters', () => {
      const xml =
        '<test>Content with &lt;special&gt; chars &amp; symbols</test>';
      const config = { targetElements: ['test'] };
      let state = parser.initializeXmlState(config);
    ...
    test('should handle very long element names', () => {
      const longElementName =
        'very_long_element_name_that_goes_on_and_on_and_on';
