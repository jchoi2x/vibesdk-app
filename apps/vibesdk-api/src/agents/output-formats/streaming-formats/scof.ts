import {
  ...
  type CodeGenerationStreamingState,
  type ParsingState,
} from '@/agents/output-formats/streaming-formats/base';
...
export class SCOFFormat extends CodeGenerationFormat {
  ...
  private isValidSCOFState(
    parsingState: any,
  ): parsingState is SCOFParsingState {
    return (
      parsingState &&
  ...
  private processAccumulatedContent(
    ...
    if (!lastLineComplete && lines.length > 0) {
      ...
      if (
        scofState.insideEofBlock &&
        scofState.eofMarker &&
        lastLine.trim() === scofState.eofMarker
      ) {
  ...
  private processLine(
    ...
    if (scofState.insideEofBlock) {
      ...
      if (scofState.currentFile && scofState.currentFileFormat) {
        ...
        if (
          this.looksLikeCommand(line) &&
          !this.isValidNestedCommand(line, scofState)
        ) {
          console.warn(
            `SCOF: Detected command-like content inside EOF block: "${line.trim()}". Treating as content.`,
          );
        }
  ...
  private tryParseFileCreation(
    ...
    for (const pattern of mismatchedQuotePatterns) {
      ...
      if (match) {
        ...
        console.warn(
          `SCOF: Auto-correcting mismatched quotes in filename: "${filename}"`,
        );
        return {
          type: 'file_creation',
          filePath: filename,
    ...
    const spacedFilenameMatch = command.match(
      /cat\s*>\s*([^<]+?)\s*<<\s*([^\s|]+)/i,
    );
    if (spacedFilenameMatch) {
      ...
      if (rawFilename.includes(' ') && !rawFilename.match(/^["'].*["']$/)) {
        console.warn(
          `SCOF: Detected unquoted filename with spaces: "${rawFilename}". Auto-correcting.`,
        );
        return {
          type: 'file_creation',
  ...
  private tryParseDiffPatch(
    ...
    const malformedPatchMatch = command.match(
      /cat\s*<<\s*([^|]+?)\s*\|\s*patch\s+(.+)/i,
    );
    ...
    const spacingIssueMatch = command.match(
      /cat\s*<<\s*([^\s|'"]+)\s*\|\s*patch\s+([^\s'"]+)/i,
    );
    if (spacingIssueMatch) {
      ...
      console.warn(
        `SCOF: Auto-correcting patch command spacing: EOF="${eofMarker}", file="${filePath}"`,
      );
      return {
        type: 'diff_patch',
        filePath: filePath,
  ...
  private handleCommand(
    ...
    if (scofState.currentFile) {
      console.warn(
        `Warning: Starting new file ${filePath} while ${scofState.currentFile} is still open`,
      );
    }
    ...
    scofState.currentFileFormat =
      type === 'file_creation' ? 'full_content' : 'unified_diff';
    ...
    if (!scofState.openedFiles.has(filePath)) {
      ...
      console.error(
        `CRITICAL RELIABILITY ERROR: Attempted to open file ${filePath} twice`,
      );
    }
  ...
  private finalizeCurrentFile(
    ...
    if (scofState.currentMode === 'diff_patch') {
      ...
      if (existingContent) {
        try {
          ...
          console.warn(
            `Failed to apply diff to ${filePath}, using raw content:`,
            error,
          );
    ...
    if (!scofState.closedFiles.has(filePath)) {
      ...
      // This should NEVER happen - log critical error if it does
      console.error(
        `CRITICAL RELIABILITY ERROR: Attempted multiple file close for ${filePath}`,
      );
    }
  ...
  private addContentLine(
    ...
    if (scofState.currentFile && scofState.currentFileFormat) {
      ...
      if (
        scofState.contentBuffer.length > 0 &&
        !scofState.contentBuffer.endsWith('\n') &&
        line.trim() !== ''
      ) {
        scofState.contentBuffer += '\n';
      }
