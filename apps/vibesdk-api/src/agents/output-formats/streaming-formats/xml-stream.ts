...
`;
  ...
  public initializeXmlState(config: XmlParsingConfig): XmlParsingState {
    return {
      ...
      targetElements: new Set(
        (config.targetElements || []).map((t) => t.toLowerCase()),
      ),
      streamingElements: new Set(
        (config.streamingElements || []).map((t) => t.toLowerCase()),
      ),
  ...
  private processXmlContent(
    ...
    const maxBufferSize =
      (state as { maxBufferSize?: number }).maxBufferSize ?? 10000;
    if (state.contentBuffer.length > maxBufferSize) {
      ...
      state.contentBuffer = state.contentBuffer.substring(
        state.contentBuffer.length - keepSize,
      );
    }
  }
  ...
  private handleClosingTag(
    ...
    if (
      state.streamingElements.has(normalizedTagName) &&
      callbacks.onElementContent
    ) {
  ...
  private storeElement(state: XmlParsingState, element: XmlElement): void {
    ...
    if (
      state.targetElements.size === 0 ||
      state.targetElements.has(normalizedTagName)
    ) {
