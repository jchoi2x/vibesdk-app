...
IMPORTANT: This reads from the VIRTUAL filesystem, NOT the sandbox. Files appear here immediately after generation and may not be deployed to sandbox yet.`,
    args: {
      command: t
        ...
        .describe(
          'Action to perform: "list" shows all files, "read" returns file contents, "delete" deletes files',
        ),
      paths: t.files
        .read()
        .optional()
    ...
    run: async ({ command, paths }) => {
      try {
        if (command === 'list') {
          ...
          logger.info('Reading files from virtual filesystem', {
            count: paths.length,
          });
          ...
          logger.info('Deleting files from virtual filesystem', {
            count: paths.length,
          });
