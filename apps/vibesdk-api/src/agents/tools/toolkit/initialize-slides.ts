import { tool, t, type } from '@/agents/tools/types';
  ...
  return tool({
    ...
    args: {
      ...
      force_preview: t
        .boolean()
        .optional()
        .describe('Force redeploy sandbox after import'),
    },
    run: async ({ theme, force_preview }) => {
      logger.info('Initializing presentation template', { theme });
      const { templateName, filesImported } = await agent.importTemplate(
        'reveal-presentation-pro',
      );
      logger.info('Imported presentation template', {
        templateName,
        filesImported,
      });
