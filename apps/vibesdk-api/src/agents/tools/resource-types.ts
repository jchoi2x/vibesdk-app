...
function buildType<T>(
  ...
  return {
    ...
    optional: () =>
      buildType(schema.optional(), (value) =>
        value === undefined ? {} : resources(value),
      ),
    default: (defaultValue) =>
      buildType(schema.default(defaultValue), resources),
  };
}
...
export const t = {
      ...
      z.array(
        z.object({
          ...
          description: z
            .string()
            .describe(
              'Brief description of what this file should do and its purpose in the project',
            ),
        }),
      ),
      (specs) => ({
        files: { mode: 'write', paths: specs.map((s) => s.path) },
      }),
    ),
  ...
  logs: {
    reset: () =>
      type(z.boolean().optional(), () => ({ sandbox: { operation: 'read' } })),
    durationSeconds: () =>
      type(z.number().optional(), () => ({ sandbox: { operation: 'read' } })),
    maxLines: () =>
      type(z.number().optional(), () => ({ sandbox: { operation: 'read' } })),
  },
