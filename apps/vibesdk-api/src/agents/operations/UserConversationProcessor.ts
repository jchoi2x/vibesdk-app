import { type ConversationalResponseType } from '@/agents/schemas';
import {
...
// import { MAX_LLM_MESSAGES } from '@/agents/constants';
...
function buildUserMessageWithContext(
  ...
  let userPrompt = USER_PROMPT.replace(
    '{{timestamp}}',
...
> {
  async execute(
    ...
    const { userMessage, conversationState, errors, images, projectUpdates } =
      inputs;
    ...
    try {
      ...
      const userPromptForInference = buildUserMessageWithContext(
        userMessage,
        errors,
        projectUpdates,
        true,
      ...
      const tools = buildTools(
        ...
        ...td,
        onStart: (
          _tc: ChatCompletionMessageFunctionToolCall,
          args: Record<string, unknown>,
        ) =>
          Promise.resolve(
            toolCallRenderer({ name: td.name, status: 'start', args }),
          ),
        onComplete: (
          _tc: ChatCompletionMessageFunctionToolCall,
          args: Record<string, unknown>,
      ...
      const runningHistory = await prepareMessagesForInference(
        env,
        conversationState.runningHistory,
      );

      const compactHistory = await compactifyContext(
        runningHistory,
        env,
        options,
        toolCallRenderer,
      ...
      if (compactHistory.length !== runningHistory.length) {
        logger.info('Conversation history compactified', {
          ...
          reduction:
            conversationState.runningHistory.length - compactHistory.length,
        });
      }
      ...
      const userPromptForHistory = buildUserMessageWithContext(
        userMessage,
        errors,
        projectUpdates,
        false,
      ...
      if (
        ...
        result.toolCallContext.messages.length > 0
      ) {
        messages.push(
      ...
      const lastToolContextMessage =
        result.toolCallContext?.messages?.[
          result.toolCallContext.messages.length - 1
        ];
      const isDuplicate =
        lastToolContextMessage?.role === 'assistant' &&
      ...
      if (compactHistory.length !== runningHistory.length) {
        ...
        const originalById = new Map(
          originalRunning.map((m) => [m.conversationId, m] as const),
        );
...
Project Updates: ${updateType}
  ...
  isProjectUpdateType(type: unknown): type is ProjectUpdateType {
    return RelevantProjectUpdateWebsoketMessages.includes(
      type as ProjectUpdateType,
    );
  }
}
