...
import {
  SCOFFormat,
  type SCOFParsingState,
} from '@/agents/output-formats/streaming-formats/scof';
import {
  IsRealtimeCodeFixerEnabled,
  RealtimeCodeFixer,
} from '@/agents/assistants/realtimeCodeFixer';
...
> {
  async execute(
    ...
    const userPrompt =
      buildPhaseImplementationUserPrompt({ phase, issues, userContext }) +
    ...
    const agentActionName = inputs.isFirstPhase
          ...
          codeGenerationFormat.parseStreamingChunks(
            ...
            (filePath: string) => {
              ...
              const originalContents =
                context.allFiles.find((f) => f.filePath === filePath)
                  ?.fileContents || '';
