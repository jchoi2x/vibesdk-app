...
export function shouldCompactify(messages: ConversationMessage[]): {
  ...
  console.log(
    `[ConversationCompactifier] shouldCompactify: turns=${turns}, estimatedTokens=${estimatedTokens}`,
  );
...
export async function compactifyContext(
  ...
  try {
    ...
    if (splitIndex <= 0) {
      logger.warn(
        'Cannot find valid turn boundary for compactification, preserving all messages',
      );
      return runningHistory;
    }
    ...
    logger.info('Compactification completed with archival', {
      ...
      compressionRatio: (
        compactifiedHistory.length / runningHistory.length
      ).toFixed(2),
      estimatedTokenSavings:
        analysis.estimatedTokens - estimateTokens(compactifiedHistory),
    ...
    logger.error('Compactification failed, preserving original messages', {
      error,
    });

    // Safe fallback: if we have too many messages, keep recent ones
    if (
      runningHistory.length >
      COMPACTIFICATION_CONFIG.PRESERVE_RECENT_MESSAGES * 3
    ) {
      const fallbackCount =
        COMPACTIFICATION_CONFIG.PRESERVE_RECENT_MESSAGES * 2;
      logger.warn(
        `Applying emergency fallback: keeping last ${fallbackCount} messages`,
      );
