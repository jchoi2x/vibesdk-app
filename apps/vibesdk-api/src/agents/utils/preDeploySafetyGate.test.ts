...
import {
  detectPreDeploySafetyFindings,
  runPreDeploySafetyGate,
} from '@/agents/utils/preDeploySafetyGate';

type PreDeploySafetyGateArgs = Parameters<typeof runPreDeploySafetyGate>[0];

function makeFile(
  partial: Partial<FileOutputType> &
    Pick<FileOutputType, 'filePath' | 'fileContents'>,
): FileOutputType {
  return {
...
function makeArgs(files: FileOutputType[]): PreDeploySafetyGateArgs {
  return {
    ...
    phase: {
      ...
      description: 'desc',
      files: [],
    } as unknown as PhaseConceptType,
  };
}
...
describe('runPreDeploySafetyGate', () => {
  ...
  it('detects selector object literal findings', () => {
    const findings = detectPreDeploySafetyFindings(
      'const x = useOS(s => ({ a: s.a }));',
    );
  ...
  it('never throws on invalid syntax; still returns files', async () => {
    ...
    await expect(runPreDeploySafetyGate(makeArgs(input))).resolves.toHaveLength(
      1,
    );
    expect(warnSpy).toHaveBeenCalled();
  });
...
describe('detectPreDeploySafetyFindings', () => {
  ...
  it('fuzz: never throws across random strings', () => {
    ...
    for (let i = 0; i < 200; i++) {
      expect(() =>
        detectPreDeploySafetyFindings(randomAscii(rng, 200)),
      ).not.toThrow();
    }
  });
});
