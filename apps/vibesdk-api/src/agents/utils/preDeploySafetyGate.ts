        ...
        if (
          ...
          if (ret) {
            if (t.isObjectExpression(ret) || t.isArrayExpression(ret)) {
              ...
              if (
                t.isIdentifier(prop) &&
                ['map', 'filter', 'reduce', 'sort', 'slice', 'concat'].includes(
                  prop.name,
                )
              ) {
                findings.push({
...
function functionBodyContainsSetState(
  fn: t.ArrowFunctionExpression | t.FunctionExpression,
): boolean {
  try {
    let found = false;
    const bodyNode: t.Node = t.isBlockStatement(fn.body)
      ? fn.body
      : t.expressionStatement(fn.body);
...
export function detectPreDeploySafetyFindings(code: string): SafetyFinding[] {
  ...
  try {
    ...
    logSafetyGateError(
      'parseCode failed in detectPreDeploySafetyFindings',
      error,
    );
    return [
      {
        message:
          'Failed to parse file for safety checks; skipping deterministic scan.',
      },
    ];
  }
...
function tryDeterministicSplitObjectSelectorDestructuring(code: string): {
  code: string;
  changed: boolean;
} {
  let ast: t.File;
  ...
  function tryRewriteVariableDeclaration(
    ...
    if (!t.isIdentifier(d.init.callee) || !d.init.callee.name.startsWith('use'))
      return null;
    ...
    if (
      !selectorArg ||
      (!t.isArrowFunctionExpression(selectorArg) &&
        !t.isFunctionExpression(selectorArg))
    )
      return null;
    ...
    for (const prop of ret.properties) {
      ...
      if (
        ...
        prop.value.object.name !== param.name
      )
        return null;
...
export async function runPreDeploySafetyGate(args: {
  ...
  try {
    ...
    for (const file of args.files) {
      ...
      const splitResult = tryDeterministicSplitObjectSelectorDestructuring(
        file.fileContents,
      );
      const afterDeterministic = splitResult.changed
        ? splitResult.code
        : file.fileContents;

      const secondFindings = detectPreDeploySafetyFindings(afterDeterministic);

      const updated: FileOutputType = splitResult.changed
        ? { ...file, fileContents: afterDeterministic }
        : file;
    ...
    try {
      realtimeCodeFixer = new RealtimeCodeFixer(
        args.env,
        args.inferenceContext,
      );
    } catch (error) {
    ...
    const fixedResults = await Promise.allSettled(
      needsFixer.map(async ({ file, findings }) => {
        ...
        try {
          ...
          logSafetyGateError('RealtimeCodeFixer.run threw', error, {
