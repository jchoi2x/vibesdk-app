...
const userPromptFormatter = (
  ...
  const variables: Record<string, string> = {
    ...
    previousFiles: previousFiles
      ? PROMPT_UTILS.serializeFiles(previousFiles, CodeSerializerType.SIMPLE)
      : '',
...
`),
        ...
        const fixResult = await executeInference({
          ...
          messages,
          modelName:
            (i !== 0 && this.altPassModelOverride) || this.lightMode
              ? this.altPassModelOverride
              : undefined,
          temperature:
            (i !== 0 && this.altPassModelOverride) || this.lightMode
              ? 0.0
        ...
        if (!fixResult) {
          this.logger.warn(
            `Realtime code fixer returned no fix for file: ${generatedFile.filePath}`,
          );
          return generatedFile;
        }
        ...
        if (fixResult.string.includes('<content>')) {
          ...
          const contentMatch = fixResult.string.match(
            /<content>([\s\S]*?)<\/content>/,
          );
          if (contentMatch) {
...
`);
      ...
      this.logger.error(
        `Error during realtime code fixer for file ${generatedFile.filePath}:`,
        error,
      );
    }
    return generatedFile;
  ...
  async applyDiffSafely(
    ...
    try {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        currentDiff = currentDiff.replaceAll(
          /^={7}\s*$\n^`{3}\s*$/gm,
          '>>>>>>> REPLACE\n```\n',
        );
        ...
        if (searchBlocks !== replaceBlocks) {
          ...
          if (!correctedDiff) {
            this.logger.warn(
              `❌ Failed to get LLM correction on attempt ${attempt + 1}`,
            );
        ...
        const { blocksApplied, blocksTotal, blocksFailed, failedBlocks } =
          result.results;
        this.logger.info(
        ...
        if (blocksFailed === 0) {
          ...
          this.logger
            .warn(`⚠️ ${blocksFailed} blocks still failed after ${attempt} retries.
                        Failed blocks:
...
-------------------------`);
        ...
        if (attempt === maxRetries) {
          this.logger.warn(
            `⚠️ ${blocksFailed} blocks still failed after ${maxRetries} retries`,
          );
        ...
        this.logger.info(
          `🔄 Getting LLM correction for ${blocksFailed} failed blocks...`,
        );
...
`,
      ...
      if (searchCount !== replaceCount) {
        this.logger.warn(
          `❌ Mismatched markers: ${searchCount} SEARCH vs ${replaceCount} REPLACE`,
          trimmed,
        );
        return null;
      }
      ...
      if (searchCount === 0) {
        this.logger.warn(
          '❌ No valid search/replace blocks found in LLM response',
          trimmed,
        );
        return null;
      }
...
export function IsRealtimeCodeFixerEnabled(
  inferenceContext: InferenceContext,
): boolean {
  ...
  if (
    ...
    inferenceContext.userModelConfigs['realtimeCodeFixer'].name !==
      AIModels.DISABLED
  ) {
  ...
  console.log(
    'Realtime code fixer disabled',
    inferenceContext.userModelConfigs,
  );
  return false;
}
