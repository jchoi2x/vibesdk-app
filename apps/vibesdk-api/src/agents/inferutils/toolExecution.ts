...
export function buildExecutionPlan(
  ...
  for (const call of toolCalls) {
    ...
    try {
      ...
      console.warn(
        `[TOOL_EXECUTION] Failed to parse arguments for ${call.function.name}:`,
        error,
      );
  ...
  while (executed.size < toolCalls.length) {
    ...
    for (const call of toolCalls) {
      ...
      const allDepsExecuted = Array.from(deps).every((depId) =>
        executed.has(depId),
      );
...
export async function executeToolCallsWithDependencies(
  ...
  console.log(
    `[TOOL_EXECUTION] Execution plan: ${plan.parallelGroups.length} parallel groups`,
  );
  plan.parallelGroups.forEach((group, i) => {
    ...
    console.log(
      `[TOOL_EXECUTION]   Group ${i + 1}: ${toolNames}${parallelIndicator}`,
    );
  });
  ...
  for (const [groupIndex, group] of plan.parallelGroups.entries()) {
    ...
    const groupResults = await Promise.all(
      group.map(async (toolCall) => {
        ...
        if (!toolDef) {
          throw new Error(
            `Tool definition not found: ${toolCall.function.name}`,
          );
        }
