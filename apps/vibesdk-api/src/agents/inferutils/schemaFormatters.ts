...
export function formatSchemaAsMarkdown<T extends z.ZodRawShape>(
  ...
  // `;
  return formatZodSchemaAsMarkdownFields(
    schema,
    headingMarker,
    undefined,
    false,
...
function formatDataAsMarkdown<T extends z.ZodRawShape>(
  ...
  if (!validation.success) {
    logger.error(
      'Input data failed Zod validation before formatting:',
      validation.error,
    );
...
function formatZodSchemaAsMarkdownFields(
  ...
  if (debug)
    logger.debug(
      `[Format Data Path: object] Formatting object with keys: ${Object.keys(shape).join(', ')}`,
    );
  ...
  for (const key of Object.keys(shape)) {
    ...
    if (debug)
      logger.debug(`[Format Data Path: object.${key}] Processing key "${key}"`);
    ...
    if (dataObject) {
      ...
      while (
        ...
        if (checkSchema instanceof z.ZodDefault && value === undefined)
          isOptional = true;
      ...
      if (
        ...
        if (debug)
          logger.debug(
            `[Format Data Path: object.${key}] Skipping optional field with undefined value.`,
          );
        continue; // Skip this field entirely
      }
    }
    result += formatZodTypeAsMarkdown(
      key,
      fieldSchema,
      headingPrefix,
      value,
...
function getMarkdownPlaceholderValue(
  key: string,
  field: z.ZodTypeAny,
  headingPrefix: string,
): string {
  ...
  if (field instanceof z.ZodArray) {
    ...
    value = field._def.checks?.some((c) => c.kind === 'int')
      ? '[Integer value]'
      : '[Numeric value]';
...
function formatZodTypeAsMarkdown(
  ...
  if (!hasValue) {
    ...
    if (mode === 'template') {
      const placeholder = getMarkdownPlaceholderValue(
        key,
        field,
        headingPrefix,
      );
    ...
    if (typeof value === 'object') {
      ...
      if (mode === 'template') {
        logger.warn(
          `[Format Data Path: ...${key}] Expected object but got ${typeof value}. Rendering empty section.`,
        );
...
function getSimpleTypeDescription(field: z.ZodTypeAny): string {
  ...
  if (
    ...
    return (
      getSimpleTypeDescription(field._def.innerType) + ' (optional/nullable)'
    );
  }
  return 'value';
}
...
function getDefaultValue(field: z.ZodTypeAny): unknown {
  ...
  if (field instanceof z.ZodDefault) {
    try {
      ...
      logger.warn(
        `Error getting default value for field: ${e instanceof Error ? e.message : String(e)}`,
      );
...
function convertToPrimitive(
  ...
  if (typeof value !== 'string') {
    ...
    if (value === undefined && schema instanceof z.ZodOptional)
      return undefined;
    ...
    if (
      ...
      if (debug)
        currentLogger.debug(
          `[Convert Path: ${pathStr}] Non-string null/undefined for ZodDefault. Returning default.`,
        );
    ...
    if (value === undefined && schema instanceof z.ZodNullable) {
      if (debug)
        currentLogger.debug(
          `[Convert Path: ${pathStr}] Non-string undefined for ZodNullable. Returning null.`,
        );
      return null;
    }
    ...
    if (baseSchema instanceof z.ZodString && typeof value === 'string')
      return value; // Already handled string case above
    if (baseSchema instanceof z.ZodNumber && typeof value === 'number')
      return value;
    if (baseSchema instanceof z.ZodBoolean && typeof value === 'boolean')
      return value;
  ...
  if (
    ...
    if (isNullish) {
      ...
      if (schema instanceof z.ZodOptional) {
        if (debug)
          currentLogger.debug(
            `[Convert Path: ${pathStr}] Empty/nullish string for ZodOptional. Returning undefined.`,
          );
        return undefined;
      }
      if (schema instanceof z.ZodNullable) {
        if (debug)
          currentLogger.debug(
            `[Convert Path: ${pathStr}] Empty/nullish string for ZodNullable. Returning null.`,
          );
        return null;
      }
    }
    ...
    if (debug)
      currentLogger.debug(
        `[Convert Path: ${pathStr}] Passing value to inner schema: ${innerSchema.constructor.name}`,
      );
  ...
  if (schema instanceof z.ZodString) {
    ...
    if (debug)
      currentLogger.debug(
        `[Convert Path: ${pathStr}] Returning string value: "${unescaped.substring(0, 100)}..."`,
      );
    return unescaped;
    ...
    if (cleanValue === '') {
      ...
      if (debug)
        currentLogger.warn(
          `[Convert Path: ${pathStr}] Empty string reached number conversion. Returning default (0).`,
        );
      return getDefaultValue(z.number()); // Use number default (0) - pass base type
    }
    const num = Number(cleanValue);
    if (debug)
      currentLogger.debug(`[Convert Path: ${pathStr}] Parsed number: ${num}`);
    ...
    if (['true', 'yes', '1', 'on'].includes(lower)) {
      // Added 'on'
      if (debug)
        currentLogger.debug(`[Convert Path: ${pathStr}] Parsed boolean: true`);
      return true;
    }
    if (['false', 'no', '0', 'off'].includes(lower)) {
      // Added 'off'
      if (debug)
        currentLogger.debug(`[Convert Path: ${pathStr}] Parsed boolean: false`);
      return false;
    }
    // This case should ideally be caught by ZodOptional/Nullable/Default handling above
    if (debug)
    ...
    const matchedEnum = enumValues.find(
      (enumVal) =>
        typeof enumVal === 'string' && enumVal.toLowerCase() === lowerValue,
    );
...
function extractPrimitiveValueFromNodes(
  ...
  if (!nodes || nodes.length === 0) {
    if (debug)
      currentLogger.debug(`[Extract Path: ${path}] No nodes provided.`);
    return '';
  }
  ...
  if (firstCodeBlock) {
    if (debug)
      currentLogger.debug(
        `[Extract Path: ${path}] Strategy 1: Found code block. Returning its value.`,
      );
  ...
  if (nodes.length === 1 && listNodes.length === 1) {
    if (debug)
      currentLogger.debug(
        `[Extract Path: ${path}] Strategy 2: Found a single list node.`,
      );
  ...
  if (debug)
    currentLogger.debug(
      `[Extract Path: ${path}] Stringified value: "${stringified.substring(0, 100)}..."`,
    );
  return stringified;
}
...
function buildSectionTree(root: Root): Section {
  ...
  for (const node of root.children as MdNode[]) {
    if (isHeading(node)) {
      ...
      while (
        stack.length > 1 &&
        (last(stack)!.heading?.depth ?? 0) >= headingNode.depth
      ) {
        stack.pop();
      }
      ...
      if (stack.length > 0) {
        ...
        logger.error(
          'Stack became empty unexpectedly during tree build for heading:',
          headingText,
        );
      ...
      if (stack.length > 0) {
        ...
        if (nodeString !== '' && !isThematicBreak(node)) {
          logger.warn(
            'Node found outside any section (before first heading?)',
            {
              nodeType: node.type,
...
function preprocessMarkdown(markdown: string, debug: boolean): string {
  ...
  if (match && match[1]) {
    ...
    if (match[1].includes('##')) {
      ...
      if (debug)
        logger.debug(
          "Outer ```...``` fences found, but content doesn't look like markdown structure. Keeping fences.",
        );
    }
  }
  ...
  if (firstHeadingIndex > 0) {
    ...
    if (!preamble.includes('\n##')) {
      ...
      if (debug)
        logger.debug(
          'Content found before first H2, but it contains other headings. Not removing preamble.',
        );
    }
  } else if (
    firstHeadingIndex === -1 &&
    ...
    if (debug)
      logger.debug(
        'No H2 headings found in input. Preprocessing might not apply.',
      );
  }
...
function getSemanticSimilarity(
  ...
  for (let n = config.ngramMinSize; n <= config.ngramMaxSize; n++) {
    ...
    if (config.weights) {
      ...
      const weight =
        config.weights[weightKey] || (n === config.ngramMaxSize ? 1 : 0); // Default to weight 1 for max N-gram if not specified
...
function mapSectionToSchema(
  ...
  const sectionHeadingText = section.heading
    ? mdastToString(section.heading)
    : '<root>'; // Use <root> for top level
  if (debug)
    logger.debug(
  ...
  if (debug && baseSchema !== schema)
    logger.debug(`[Map Path: ${currentPath}] Schema Info: ${schemaInfo}`);
  ...
  if (baseSchema instanceof z.ZodObject) {
    ...
    if (debug)
      logger.debug(
        `[Map Path: ${currentPath}] Initialized object with defaults`,
        {
          path: currentPath,
      ...
      if (childSection.heading) {
        ...
        for (const key of Object.keys(remainingKeys)) {
          ...
          if (keyDistance < minDistance) {
            ...
            normalize(key) === normalizedChildHeading &&
            keyDistance === 0
          ) {
        ...
        if (matchedKey) {
          ...
          if (debug)
            logger.debug(
              `[Map Path: ${currentPath}] Normalized heading does not match schema shape`,
              {
                path: currentPath,
        ...
        if (debug)
          logger.debug(
            `[Map Path: ${currentPath}]  -- Skipping child section with no heading.`,
          );
      }
    }
    if (debug)
  ...
  if (baseSchema instanceof z.ZodArray) {
    ...
    const itemHeadingRegex = new RegExp(
      `^${singularKeyName.toLowerCase()}(?:\\s*(?:#|\\d+))?$`,
      'i',
    );
    ...
    if (directList && !(itemSchema instanceof z.ZodObject)) {
      ...
      directList.children.forEach((listItem, i) => {
        if (isListItem(listItem)) {
          ...
          const itemSection: Section = {
            heading: null,
            nodes: listItem.children as MdNode[],
            children: [],
          };
          if (debug)
            logger.debug(
              `[Map Path: ${itemPath.join('.')}] Mapping list item ${i}.`,
            );
      ...
      if (results.length > 0) {
        if (debug)
          logger.debug(
            `[Map Path: ${currentPath}] Returning ${results.length} items from direct list.`,
          );
        return results;
      }
    }
    ...
    for (const childSection of section.children) {
      if (childSection.heading) {
        ...
        if (debug)
          logger.debug(
            `[ItemMatching Path: ${itemMatchPath}]   - Child section has no heading. Skipping.`,
          );
      }
    }
    ...
    for (const node of section.nodes) {
      if (isThematicBreak(node)) {
        ...
        const sliceContent = mdastToString({
          type: 'root',
          children: currentSlice,
        }).trim();
        if (
          currentSlice.length > 0 &&
          sliceContent !== '' &&
          !sliceContent.startsWith('*Description:')
        ) {
    ...
    const lastSliceContent = mdastToString({
      type: 'root',
      children: currentSlice,
    }).trim();
    if (
      currentSlice.length > 0 &&
      lastSliceContent !== '' &&
      !lastSliceContent.startsWith('*Description:')
    ) {
      slices.push(currentSlice);
    }
    ...
    if (slices.length > 0) {
      if (debug)
        logger.debug(
          `[Map Path: ${currentPath}] Strategy 3: Found ${slices.length} items based on thematic breaks.`,
        );
      return slices.map((sliceNodes, i) => {
        ...
        if (debug)
          logger.debug(
            `[Map Path: ${itemPath.join('.')}] Mapping thematic break slice ${i}.`,
          );
        return mapSectionToSchema(
          itemSection,
          itemSchema,
    ...
    if (!(itemSchema instanceof z.ZodObject)) {
      if (debug)
        logger.debug(
          `[Map Path: ${currentPath}] Strategy 4: Handling array of primitives fallback.`,
        );
      ...
      if (!contentString) {
        if (debug)
          logger.debug(
            `[Map Path: ${currentPath}] No content found for primitive array. Returning empty array.`,
          );
        return [];
      }
      ...
      if (looksLikeList) {
        if (debug)
          logger.debug(
            `[Map Path: ${currentPath}] Parsing primitive array from ${lines.length} list-like lines.`,
          );
    ...
    if (debug)
      logger.debug(
        `[Map Path: ${currentPath}] No array items found using any strategy. Returning empty array.`,
      );
    return [];
  }
  ...
  if (debug)
    logger.debug(
      `[Map Path: ${currentPath}] Handling Primitive/Other Schema (${baseSchema.constructor.name}).`,
    );
  if (debug) {
    ...
    logger.debug(
      ...
      section.nodes.map((n) => ({
        type: n.type,
        value:
          (n as any).value?.substring(0, 30) ??
          mdastToString(n).substring(0, 30) + '...',
      })),
    );
  }
  ...
  if (debug)
    logger.debug(
      `[Map Path: ${currentPath}] Converted value: ${JSON.stringify(convertedValue)}`,
    );
  ...
  if (
    ...
    if (!(schema instanceof z.ZodNullable)) {
      if (debug)
        logger.warn(
          `[Map Path: ${currentPath}] CONVERTED UNDEFINED for non-optional/non-default field.`,
        );
    }
  }
  if (
    ...
    if (!(schema instanceof z.ZodOptional)) {
      if (debug)
        logger.warn(
          `[Map Path: ${currentPath}] CONVERTED NULL for non-nullable/non-default field.`,
        );
    }
  }
