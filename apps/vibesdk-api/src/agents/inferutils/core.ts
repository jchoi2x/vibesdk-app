...
type ToolAccumulatorEntry = ChatCompletionMessageFunctionToolCall & {
  index?: number;
  __order: number;
};
...
function accumulateToolCallDelta(
  ...
  if (idFromDelta && byId.has(idFromDelta)) {
    ...
    console.log(
      `[TOOL_CALL_DEBUG] Creating new entry - id: ${idFromDelta}, index: ${idx}`,
    );
    // Create new entry
  ...
  if (deltaToolCall.function?.arguments !== undefined) {
    ...
    if (!isComplete) {
      ...
      console.log(
        `[TOOL_CALL_DEBUG] Accumulating arguments for ${entry.function.name || 'unknown'}:`,
        {
          id: entry.id,
          index: entry.index,
...
function buildGatewayPathname(
  cleanPathname: string,
  providerOverride?: AIGatewayProviders,
): string {
  return providerOverride
    ? `${cleanPathname}/${providerOverride}`
    : `${cleanPathname}/compat`;
}
...
export async function buildGatewayUrl(
  ...
  const baseUrl = providerOverride
    ? await gateway.getUrl(providerOverride)
    : `${await gateway.getUrl()}compat`;
  return baseUrl;
}
...
async function getApiKey(
  ...
  if (!isValidApiKey(apiKey)) {
    ...
    if (runtimeOverrides?.aiGatewayOverride?.baseUrl) {
      ...
      apiKey =
        runtimeOverrides?.aiGatewayOverride?.token ??
...
export async function getConfigurationForModel(
  ...
  const baseURL = await buildGatewayUrl(
    env,
    providerForcedOverride,
    gatewayOverride,
  );
  ...
  try {
      ...
      ? {
          extra_body: {
            thinking: {
              type: 'enabled',
              budget_tokens:
                claude_thinking_budget_tokens[reasoning_effort ?? 'medium'],
            },
          },
        }
      : {};
    ...
    if (toolCallContext && toolCallContext.messages) {
      ...
      let filtered = ctxMessages.filter((msg) => {
        ...
        if (msg.role === 'tool') {
          if (!msg.name?.trim()) {
            console.warn(
              '[TOOL_ORPHAN] Dropping tool message with empty name:',
              msg.tool_call_id,
            );
            return false;
          }
          if (!msg.tool_call_id || !validToolCallIds.has(msg.tool_call_id)) {
            console.warn(
              '[TOOL_ORPHAN] Dropping orphaned tool message:',
              msg.name,
              msg.tool_call_id,
            );
            return false;
    ...
    if (format) {
      if (!schema || !schemaName) {
        throw new Error(
          'Schema and schemaName are required when using a custom format',
        );
      }
      ...
      response = await client.chat.completions.create(
        {
          ...
          reasoning_effort: modelConfig.nonReasoning
            ? undefined
            : reasoning_effort,
          temperature,
          frequency_penalty,
    ...
    if (stream) {
      ...
      if (response instanceof Stream) {
        ...
        for await (const event of response) {
          ...
          if (
            delta?.tool_calls &&
            (provider === 'google-ai-studio' || provider === 'gemini')
          ) {
            console.log(
              `[PROVIDER_DEBUG] ${provider} tool_calls delta:`,
          ...
          const finishReason = (event as ChatCompletionChunk).choices[0]
            ?.finish_reason;
        ...
        const dropped = assembled.filter(
        ...
        console.error(
          'Expected a stream response but received a ChatCompletion object.',
        );
      ...
      const droppedNonStream = allToolCalls.filter(
        (tc) => !tc.function.name || tc.function.name.trim() === '',
      );
      ...
      const totalTokens = (response as OpenAI.ChatCompletion).usage
        ?.total_tokens;
    ...
    if (tools) {
      ...
      try {
        ...
        if (error instanceof AbortError) {
          console.warn(
            `Tool call was aborted, ending tool call chain with the latest tool call result`,
          );
    ...
    if (executedToolCalls.length) {
      console.log(
        `Tool calls executed:`,
        JSON.stringify(executedToolCalls, null, 2),
      );
      ...
      if (newToolCallContext.completionSignal?.signaled) {
        console.log(
          `[COMPLETION] ${newToolCallContext.completionSignal.toolName} called, stopping recursion`,
        );
        ...
        return {
          string:
            content ||
            newToolCallContext.completionSignal.summary ||
            'Task complete',
      ...
      console.log(
        `${actionKey}: Tool depth ${newToolCallContext.depth}/${getMaxToolCallingDepth(actionKey)}`,
      );
    ...
    try {
      ...
      if (!result.success) {
        ...
        throw new Error(
          `Failed to validate AI response against schema: ${result.error.message}`,
        );
      }
