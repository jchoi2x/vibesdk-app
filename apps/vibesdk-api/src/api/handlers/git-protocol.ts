...
async function verifyGitAccess(
  ...
  logger.info('App details retrieved', {
    appId,
    found: !!app,
  ...
  if (!token) {
    logger.warn(
      'No token found in auth header - will return 401 to prompt git for credentials',
    );
    return { hasAccess: false };
  }
  ...
  if (!hasAccess) {
    logger.warn('Access denied - user does not own this app', {
      userId: payload.sub,
      appOwnerId: app.userId,
    });
  } else {
  ...
  return {
    hasAccess,
    appCreatedAt: hasAccess ? app.createdAt || undefined : undefined,
  };
}
...
async function handleInfoRefs(
  ...
  try {
    // Verify access first
    const { hasAccess, appCreatedAt } = await verifyGitAccess(
      request,
      env,
      appId,
    );
    ...
    const { gitObjects, query, hasCommits, templateDetails } =
    ...
    const { repo } = await cache.getRepository(
      appId,
      agentHeadOid,
      templateDetails,
    );
...
async function handleUploadPack(
  ...
  try {
    ...
    const cache = new GitCache();
    const { repo, source } = await cache.getRepository(
      appId,
      agentHeadOid,
      templateDetails,
    );
    ...
    if (repo) {
      logger.info(`Cache HIT (${source}): upload-pack`, {
        appId,
        agentHeadOid,
      });
