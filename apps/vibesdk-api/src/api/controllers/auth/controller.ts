...
import {
  ...
  registerSchema,
  oauthProviderSchema,
} from '@/api/controllers/auth/authSchemas';
...
export class AuthController extends BaseController {
  ...
  static async initiateOAuth(
    ...
    try {
      const validatedProvider = oauthProviderSchema.parse(
        routeContext.pathParams.provider,
      );

      // Get intended redirect URL from query parameter
      const intendedRedirectUrl =
        routeContext.queryParams.get('redirect_url') || undefined;
  ...
  static async createApiKey(
    ...
    try {
      ...
      const bodyResult = await AuthController.parseJsonBody<{ name?: string }>(
        request,
      );
      if (!bodyResult.success) {
        return bodyResult.response!;
  ...
  static async resendVerificationOtp(
    ...
    try {
      const bodyResult = await AuthController.parseJsonBody<{ email: string }>(
        request,
      );
      if (!bodyResult.success) {
        return bodyResult.response!;
  ...
  static async getAuthProviders(
    ...
    try {
      ...
      return AuthController.createErrorResponse(
        'Failed to get authentication providers',
        500,
      );
    }
  }
