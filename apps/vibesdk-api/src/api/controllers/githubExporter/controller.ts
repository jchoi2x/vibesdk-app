...
export class GitHubExporterController extends BaseController {
  ...
  private static async verifyState(
    token: string,
    secret: string,
  ): Promise<GitHubExportStatePayload | null> {
    try {
    ...
    try {
      ...
      if (!finalExistingRepoUrl) {
        try {
          ...
          this.logger.warn('Failed to check for existing repository', {
            error,
            agentId,
          });
        }
      ...
      if (finalExistingRepoUrl) {
        ...
        if (!exists) {
          ...
          try {
            ...
            this.logger.warn('Failed to clear repository URL', {
              error: clearError,
              agentId,
            });
          }
      ...
      if (!finalExistingRepoUrl) {
        this.logger.info('Creating new repository', {
          agentId,
          repositoryName,
        });
        ...
        if (!createResult.success) {
          ...
          if (createResult.alreadyExists && createResult.repositoryName) {
            this.logger.warn(
              'Repository already exists, fetching repository info',
              {
                agentId,
          ...
          return {
            success: false,
            error: createResult.error || 'Repository creation failed',
          };
        }
      ...
      if (!pushResult?.success) {
        return {
          ...
          error:
            pushResult?.error ||
            (finalExistingRepoUrl ? 'Sync failed' : 'File push failed'),
        };
      }

      const operationType = finalExistingRepoUrl ? 'Sync' : 'Export';
      this.logger.info(`${operationType} completed`, {
        agentId,
        repositoryUrl,
      });
  ...
  static async handleOAuthCallback(
    ...
    try {
      ...
      const validatedReturnUrl =
        validateRedirectUrl(returnUrl, request) ||
      ...
      if (purpose === 'repository_export') {
        const appService = new AppService(env);
        const ownershipResult = await appService.checkAppOwnership(
          agentId,
          userId,
        );
        if (!ownershipResult.isOwner) {
          this.logger.warn('OAuth callback ownership check failed', {
            userId,
            agentId,
          });
          return Response.redirect(
        ...
        this.logger.info('OAuth export completed', {
          userId,
          agentId,
  ...
  static async initiateGitHubExport(
    ...
    try {
      ...
      this.logger.info('Export initiated', {
      ...
      if (cachedToken) {
        ...
        if (isTemporaryError) {
          this.logger.warn('Temporary error, keeping token', {
            error: result.error,
            agentId: body.agentId,
          });
        ...
        this.logger.info('No cached token, initiating OAuth', {
          agentId: body.agentId,
        });
      }
      ...
      const validatedReturnUrl =
        validateRedirectUrl(rawReturnUrl, request) || `${baseUrl}/chat`;
  ...
  static async checkRemoteStatus(
    ...
    try {
      ...
      const { gitObjects, query, templateDetails } =
        await agentStub.exportGitObjects();
      ...
      try {
        ...
        this.logger.warn('Failed to get app createdAt for sync check', {
          error,
        });
      }
