...
export class AppController extends BaseController {
  ...
  static async getUserApps(
    ...
    try {
      ...
      return AppController.createErrorResponse<AppsListData>(
  ...
  static async getRecentApps(
    ...
    try {
      ...
      const appService = new AppService(env);
      const recentApps = await appService.getRecentAppsWithFavorites(
        user.id,
        10,
      );
  ...
  static async toggleFavorite(
    ...
    try {
      ...
      if (!appId) {
        return AppController.createErrorResponse<FavoriteToggleData>(
          'App ID is required',
          400,
        );
      }
    ...
    try {
      ...
      const responseData: PublicAppsData = {
        apps: result.data.map((app) => ({
          ...
          createdAtFormatted: app.createdAt
      ...
      return AppController.createErrorResponse<PublicAppsData>(
        'Failed to fetch public apps',
        500,
      );
    }
  };
  ...
  static async getApp(
    ...
    try {
      ...
      const appId = context.pathParams.id;
      if (!appId) {
        return AppController.createErrorResponse<SingleAppData>(
          'App ID is required',
          400,
        );
      }
      ...
      const appService = new AppService(env);
      const app = await appService.getSingleAppWithFavoriteStatus(
        appId,
        user.id,
      );
    ...
    try {
      ...
      if (!bodyResult.success) {
        return bodyResult.response! as ControllerResponse<
          ApiResponse<UpdateAppVisibilityData>
        >;
      }

      const visibility = (bodyResult.data as { visibility?: string })
        ?.visibility;
      ...
      const appService = new AppService(env);
      const result = await appService.updateAppVisibility(
        appId,
        user.id,
        validVisibility,
      );
      ...
      return AppController.createErrorResponse<UpdateAppVisibilityData>(
        'Failed to update app visibility',
        500,
      );
    }
  }
  ...
  static async deleteApp(
    ...
    try {
      ...
      const appId = context.pathParams.id;
      if (!appId) {
        return AppController.createErrorResponse<AppDeleteData>(
          'App ID is required',
          400,
        );
      }
      ...
      if (!result.success) {
        const statusCode =
          result.error === 'App not found'
            ? 404
            : result.error?.includes('only delete your own apps')
              ? 403
              : 500;
        return AppController.createErrorResponse<AppDeleteData>(
