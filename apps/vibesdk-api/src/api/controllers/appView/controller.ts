...
export class AppViewController extends BaseController {
  ...
  static async getAppDetails(
    ...
    try {
      ...
      if (!appId) {
        return AppViewController.createErrorResponse<AppDetailsData>(
          'App ID is required',
          400,
        );
      }
      ...
      try {
        ...
        this.logger.warn(
          'Could not fetch agent state, using stored files:',
          agentError,
        );
      }

      const cloudflareUrl = appResult.deploymentId
        ? buildUserWorkerUrl(env, appResult.deploymentId)
        : '';
  ...
  static async toggleAppStar(
    ...
    try {
      ...
      const appId = context.pathParams.id;
      if (!appId) {
        return AppViewController.createErrorResponse<AppStarToggleData>(
          'App ID is required',
          400,
        );
      }
  ...
  static async generateGitCloneToken(
    ...
    try {
      ...
      if (!appId) {
        return AppViewController.createErrorResponse<GitCloneTokenData>(
          'App ID is required',
          400,
        );
      }
