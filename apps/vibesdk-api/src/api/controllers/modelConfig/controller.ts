...
import {
  ...
  type ModelConfigUpdateData,
  type ModelConfigTestData,
  type ModelConfigResetData,
...
export class ModelConfigController extends BaseController {
  ...
  static async getModelConfigs(
    ...
    try {
      ...
      return ModelConfigController.createErrorResponse<ModelConfigsData>(
  ...
  static async getModelConfig(
    ...
    try {
      ...
      if (!agentAction || !(agentAction in AGENT_CONFIG)) {
        return ModelConfigController.createErrorResponse<ModelConfigData>(
      ...
      const config = await modelConfigService.getUserModelConfig(
        user.id,
        agentAction,
      );
    ...
    try {
      ...
      if (!bodyResult.success) {
        return bodyResult.response! as ControllerResponse<
          ApiResponse<ModelConfigUpdateData>
        >;
      }
      ...
      if (
        validatedData.modelName !== null &&
        validatedData.modelName !== undefined
      ) {
        modelConfig.name = validatedData.modelName;
      }
      if (
        validatedData.maxTokens !== null &&
        validatedData.maxTokens !== undefined
      ) {
        modelConfig.max_tokens = validatedData.maxTokens;
      }
      if (
        validatedData.temperature !== null &&
        validatedData.temperature !== undefined
      ) {
        modelConfig.temperature = validatedData.temperature;
      }
      if (
        validatedData.reasoningEffort !== null &&
        validatedData.reasoningEffort !== undefined
      ) {
        modelConfig.reasoning_effort = validatedData.reasoningEffort;
      }
      if (
        validatedData.fallbackModel !== null &&
        validatedData.fallbackModel !== undefined
      ) {
  ...
  static async deleteModelConfig(
    ...
    try {
      ...
      const modelConfigService = new ModelConfigService(env);
      const deleted = await modelConfigService.deleteUserModelConfig(
        user.id,
        agentAction,
      );
  ...
  static async testModelConfig(
    ...
    try {
      ...
      if (!bodyResult.success) {
        return bodyResult.response! as ControllerResponse<
          ApiResponse<ModelConfigTestData>
        >;
      }
      ...
      const baseConfig = await modelConfigService.getUserModelConfig(
        user.id,
        agentAction,
      );
      ...
      const configToTest: ModelConfig = validatedData.tempConfig
        ? {
            ...
            ...(validatedData.tempConfig.modelName != null && {
              name: validatedData.tempConfig.modelName,
            }),
      ...
      return ModelConfigController.createErrorResponse<ModelConfigTestData>(
        'Failed to test model configuration',
        500,
      );
    }
  }
  ...
  static async getByokProviders(
    ...
    try {
      ...
      if (
        agentActionParam &&
        validAgentActions.includes(agentActionParam as AgentActionKey)
      ) {
      ...
      if (agentAction) {
        ...
        const combinedModels = [
          ...new Set([...allAccessibleModels, ...allPlatformModels]),
        ];

        // Filter based on agent constraints
        const filteredModels = getFilteredModelsForAgent(
          agentAction,
          combinedModels,
        );
        ...
        const filteredPlatformModels = allPlatformModels.filter((m) =>
          filteredSet.has(m),
        );
