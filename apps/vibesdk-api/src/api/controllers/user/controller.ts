...
} from '@/api/controllers/user/types';
...
export class UserController extends BaseController {
  ...
  static async getApps(
    ...
    try {
      ...
      return UserController.createErrorResponse<UserAppsData>(
        'Failed to get user apps',
        500,
      );
    }
  }
  ...
  static async updateProfile(
    ...
    try {
      ...
      const result = await userService.updateUserProfileWithValidation(
        user.id,
        bodyResult.data!,
      );

      if (!result.success) {
        return UserController.createErrorResponse<ProfileUpdateData>(
          result.message,
          400,
        );
      }
