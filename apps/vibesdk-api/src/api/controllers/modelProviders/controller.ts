...
import {
  ...
  type ModelProviderCreateData,
  type ModelProviderUpdateData,
  type ModelProviderDeleteData,
  type ModelProviderTestData,
...
export class ModelProvidersController extends BaseController {
  ...
  static async getProviders(
    ...
    try {
      ...
      return ModelProvidersController.createErrorResponse<ModelProvidersListData>(
  ...
  static async getProvider(
    ...
    try {
      ...
      if (!providerId) {
        return ModelProvidersController.createErrorResponse<ModelProviderData>(
      ...
      const modelProvidersService = new ModelProvidersService(env);
      const provider = await modelProvidersService.getProvider(
        user.id,
        providerId,
      );
  ...
  static async createProvider(
    ...
    try {
      const bodyResult =
        await ModelProvidersController.parseJsonBody<CreateProviderRequest>(
          request,
        );
      if (!bodyResult.success) {
        return bodyResult.response as ControllerResponse<
          ApiResponse<ModelProviderCreateData>
        >;
      }
  ...
  static async updateProvider(
    ...
    try {
      ...
      const bodyResult =
        await ModelProvidersController.parseJsonBody<UpdateProviderRequest>(
          request,
        );
      if (!bodyResult.success) {
        return bodyResult.response as ControllerResponse<
          ApiResponse<ModelProviderUpdateData>
        >;
      }
  ...
  static async testProvider(
    ...
    try {
      const bodyResult =
        await ModelProvidersController.parseJsonBody<TestProviderRequest>(
          request,
        );
      if (!bodyResult.success) {
        return bodyResult.response as ControllerResponse<
          ApiResponse<ModelProviderTestData>
        >;
      }
