...
export abstract class BaseController {
  ...
  static createSuccessResponse<T>(data: T): ControllerResponse<ApiResponse<T>> {
    const response = successResponse(data) as ControllerResponse<
      ApiResponse<T>
    >;
  ...
  static createErrorResponse<T = never>(
    ...
    const response = errorResponse(message, statusCode) as ControllerResponse<
      ApiResponse<T>
    >;
    return response;
  }
