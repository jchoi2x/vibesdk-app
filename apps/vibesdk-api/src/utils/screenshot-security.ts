...
export class ScreenshotSecurity {
  ...
  async signUrl(url: string, appId: string): Promise<string> {
    ...
    const token = await this.jwt.signPayload(
      { appId, purpose: 'screenshot' },
      ScreenshotSecurity.TOKEN_EXPIRY,
    );
    parsed.searchParams.set('token', token);
    return isAbsolute ? parsed.toString() : parsed.pathname + parsed.search;
  }

  async enrichUrls<T extends { id: string; screenshotUrl?: string | null }>(
    apps: T[],
  ): Promise<T[]> {
    return Promise.all(
      apps.map(async (app) => {
        ...
        return signed === app.screenshotUrl
          ? app
          : { ...app, screenshotUrl: signed };
      }),
    );
  }
  ...
  async verifyToken(token: string, expectedAppId: string): Promise<boolean> {
    ...
    return (
      payload?.appId === expectedAppId && payload?.purpose === 'screenshot'
    );
  }
}
