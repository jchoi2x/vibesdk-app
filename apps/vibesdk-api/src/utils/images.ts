...
export async function uploadImageToCloudflareImages(
  ...
  if (!resp.ok || !json.success || !json.result) {
    const errMsg =
      json.errors?.map((e) => e.message).join('; ') || `status ${resp.status}`;
...
export async function uploadImage(
  ...
  if (env.USE_CLOUDFLARE_IMAGES) {
    try {
      cfImagesUrl = await uploadImageToCloudflareImages(
        env,
        image,
        type,
        bytes,
  ...
  const { r2Key, url } = await uploadImageToR2(
    env,
    image,
    type,
    cfImagesUrl,
...
export async function imagesToBase64(
  ...
  return (
    await Promise.all(images.map((image) => imageToBase64(env, image)))
  ).filter((image) => image !== '');
}

export async function downloadR2Image(
  env: Env,
  r2Key: string,
): Promise<ProcessedImageAttachment> {
  ...
  const mimeType = response.httpMetadata!
    .contentType! as SupportedImageMimeType;
