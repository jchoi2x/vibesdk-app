...
export class AppService extends BaseService {
  ...
  async getPublicApps(
  ...
  ): Promise<PaginatedResult<EnhancedAppData>> {
    const {
      limit = 20,
      offset = 0,
      sort = 'recent',
    ...
    try {
      ...
      const { userStars, userFavorites } = await this.addUserSpecificAppData(
        appIds,
        userId,
      );

      const appsWithAnalytics: EnhancedAppData[] = basicApps.map(
        (row: RankedAppQueryResult) => {
  ...
  async getAppDetails(
    ...
    const [viewCount, starCount, isFavorite, userHasStarred] =
      await Promise.all([
        // View count
        readDb
          .select({ count: sql<number>`count(*)` })
  ...
  async toggleAppStar(
    ...
    const existingStar = await this.database
      ...
      .from(schema.stars)
      .where(
        and(eq(schema.stars.userId, userId), eq(schema.stars.appId, appId)),
      )
      .get();
   ...
   */
  async getUserAppsWithAnalytics(
    userId: string,
    options: Partial<AppQueryOptions> = {},
  ...
  private async executeRankedQuery(
    ...
    if (sort === 'trending' || sort === 'popular') {
      const periodThreshold =
        sort === 'trending' ? this.getTimePeriodThreshold(period) : null;
      const periodUnixTimestamp = periodThreshold
        ? Math.floor(periodThreshold.getTime() / 1000)
        : 0;
  ...
  async deleteApp(
    ...
    try {
      ...
      if (deleteResult.length === 0) {
        return {
          success: false,
          error: 'Failed to delete app - app may have been already deleted',
        };
      }
  ...
  private async enrichScreenshotUrls<
    T extends { id: string; screenshotUrl?: string | null },
  >(apps: T[]): Promise<T[]> {
