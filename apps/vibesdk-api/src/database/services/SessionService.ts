...
export class SessionService extends BaseService {
  ...
  async createSession(
    ...
    try {
      ...
      const expiresAt = new Date(
        Date.now() + SessionService.config.sessionTTL * 1000,
      );
    ...
    try {
      ...
      if (sessions.length > SessionService.config.maxSessions) {
        const sessionsToDelete = sessions.slice(
          SessionService.config.maxSessions,
        );
    ...
    try {
      ...
      if (activeSessionCount > SessionService.config.maxConcurrentDevices) {
        ...
        recommendations.push(
          'Consider revoking old sessions - you have many active sessions',
        );
      }
      ...
      if (recentSecurityEvents > 5) {
        riskLevel = 'high';
        recommendations.push(
          'Multiple security events detected - review your account activity',
        );
      } else if (recentSecurityEvents > 2) {
        riskLevel = 'medium';
        recommendations.push(
          'Some suspicious activity detected - monitor your account',
        );
      }

      // Check for session hijacking events
      const hijackingEvents = recentEvents.filter(
        (e) => e.action === 'session_hijacking',
      );
      if (hijackingEvents.length > 0) {
        riskLevel = 'high';
        recommendations.push(
          'Session hijacking attempts detected - change your password immediately',
        );
      }
   ...
   */
  async forceLogoutAllOtherSessions(
    userId: string,
    ...
    try {
      ...
      logger.info('Force logged out other sessions', {
        userId,
        currentSessionId,
        deletedCount,
      });
