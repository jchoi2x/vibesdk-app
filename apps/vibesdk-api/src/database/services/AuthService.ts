...
import { type AuthUser, type OAuthProvider } from '@/types/auth-types';
...
export class AuthService extends BaseService {
  ...
  async register(
    ...
    try {
      ...
      logger.info('User registered and logged in directly', {
        userId,
        email: data.email,
      });
  ...
  async handleOAuthCallback(
    ...
    try {
      ...
      const { accessToken: sessionAccessToken, session } =
        await this.sessionService.createSession(user.id, request);
