...
type ResolvedMonitoringOptions = Omit<
  ...
  async acquire(): Promise<boolean> {
    for (let attempt = 0; attempt < FileLock.MAX_RETRIES; attempt++) {
      try {
        ...
        await fs.writeFile(this.lockFilePath, `${process.pid}:${Date.now()}`, {
          flag: 'wx',
        });
        this.acquired = true;
        return true;
        ...
        if (fsError?.code === 'EEXIST') {
          ...
          await new Promise((resolve) =>
            setTimeout(
              resolve,
              FileLock.RETRY_DELAY_MS + Math.random() * FileLock.RETRY_DELAY_MS,
            ),
          );
        } else {
          throw error;
...
export class ProcessMonitor extends EventEmitter {
  ...
  public async start(): Promise<Result<ProcessInfo>> {
    try {
      ...
      const fullCommand =
        `${this.processInfo.command} ${this.processInfo.args?.join(' ') || ''}`.trim();
      ...
      if (this.lastSuccessfulStart && this.processInfo.endTime) {
        const runDuration =
          this.processInfo.endTime.getTime() -
          this.lastSuccessfulStart.getTime();
        if (runDuration > ProcessMonitor.STABILITY_THRESHOLD_MS) {
          console.log(
            `Previous run was stable (${Math.round(runDuration / 1000)}s), resetting restart count`,
          );
          await this.simpleLogManager
            .appendMonitorLog(
      ...
      if (!this.childProcess.pid) {
        await this.simpleLogManager
          .appendMonitorLog(`Process spawn failed: no PID assigned`)
          .catch(() => {});
      ...
      const portInfo = this.options.expectedPort
        ? `, expectedPort=${this.options.expectedPort}`
        : '';
      await this.simpleLogManager
        .appendMonitorLog(
      ...
      await this.simpleLogManager
        .appendMonitorLog(`Process start failed: ${errorMessage}`)
        .catch(() => {});
  ...
  public async stop(): Promise<Result<void>> {
    try {
      ...
      if (this.restartTimer) {
        ...
        await this.simpleLogManager
          .appendMonitorLog(`Cancelled pending restart`)
          .catch(() => {});
      }
      ...
      if (this.state === 'stopping') {
        ...
        await this.simpleLogManager
          .appendMonitorLog(`Process stopped gracefully`)
          .catch(() => {});
      ...
      await this.simpleLogManager
        .appendMonitorLog(`Stop failed: ${errorMessage}`)
        .catch(() => {});
  ...
  private flushStreamBuffers(): void {
    if (this.stdoutBuffer.trim()) {
      this.simpleLogManager
        .appendLog(this.stdoutBuffer.trim(), 'stdout')
        .catch(() => {});
    ...
    if (this.stderrBuffer.trim()) {
      this.simpleLogManager
        .appendLog(this.stderrBuffer.trim(), 'stderr')
        .catch(() => {});
      const logLine: LogLine = {
  ...
  private setupProcessMonitoring(): void {
    ...
    this.childProcess.on('error', (error) => {
      ...
      this.simpleLogManager
        .appendMonitorLog(`Process spawn/runtime error: ${error.message}`)
        .catch(() => {});
      ...
      this.simpleLogManager
        .appendLog(`Process error: ${error.message}`, 'stderr')
        .catch(() => {});
  ...
  private parseJsonLog(line: string): void {
    try {
      ...
      if (logData.level && logData.level >= 50) {
        ...
        const simpleError: SimpleError = {
          timestamp: logData.time
            ? new Date(logData.time).toISOString()
        ...
        if (storeResult.success) {
          console.log(
            `Error detected (level ${logData.level}): ${message.substring(0, 100)}...`,
          );
      ...
      if (line.startsWith('{') && line.endsWith('}')) {
        console.debug(
          `Failed to parse JSON log line: ${line.substring(0, 100)}...`,
        );
      }
    }
  }
  ...
  private shouldRestartAfterExit(
    ...
    if (this.restartCount >= this.options.maxRestarts) {
      console.error(
        `Max restart attempts (${this.options.maxRestarts}) reached`,
      );
      return false;
    }
  ...
  private scheduleRestart(): void {
    ...
    this.restartTimer = setTimeout(async () => {
      ...
      console.log(
        `Restarting process (attempt ${this.restartCount}/${this.options.maxRestarts})...`,
      );
    ...
    this.healthCheckTimer = setInterval(() => {
      if (this.state === 'running') {
        this.performHealthCheck().catch(async (error) => {
          ...
          await this.simpleLogManager
            .appendMonitorLog(`Health check error: ${message}`)
            .catch(() => {});
        });
      }
  ...
  private async performHealthCheck(): Promise<void> {
    ...
    if (this.options.expectedPort) {
      ...
      const timeSinceStartMs = this.processInfo.startTime
        ? now.getTime() - this.processInfo.startTime.getTime()
        : 0;
      ...
      if (portResult.responsive) {
        if (!this.portBindConfirmed) {
          ...
          await this.simpleLogManager
            .appendMonitorLog(`Port ${port} is now accepting connections`)
            .catch(() => {});
        }
        ...
        if (this.portFailureCount > 0) {
          await this.simpleLogManager
            .appendMonitorLog(
              `Port ${port} is responsive again (failures cleared)`,
            )
            .catch(() => {});
        }
        ...
        const shouldCountFailure =
          ...
          timeSinceStartMs >= ProcessMonitor.PORT_STARTUP_GRACE_MS;

        if (shouldCountFailure) {
          this.portFailureCount += 1;

          const errorSuffix = portResult.error
    ...
    if (timeSinceLastActivity > this.options.healthCheckInterval * 2) {
      healthIssues.push(
        `No output for ${Math.round(timeSinceLastActivity / 1000)}s`,
      );
    }
    ...
    if (healthIssues.length > 0) {
      ...
      await this.simpleLogManager
        .appendMonitorLog(`Health check warning: ${issueList}`)
        .catch(() => {});
   ...
   */
  private async checkPortResponsive(
    port: number,
  ): Promise<{ responsive: boolean; error?: string }> {
