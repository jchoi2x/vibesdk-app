...
export interface IRateLimitState {
    buckets: Map<string, IRateLimitBucket>;
    lastCleanup: number;
}
...
export interface IRateLimitStore {
  increment(key: string, config: IRateLimitConfig, incrementBy: number): Promise<IRateLimitResult>;
  getRemainingLimit(key: string, config: IRateLimitConfig): Promise<number>;
  resetLimit(key?: string): Promise<void>;
}

/**
 * RateLimitStore - Durable Object-based rate limiting store
 *
 ...
 */
export class RateLimitStore extends DurableObject<Env> implements IRateLimitStore {
    private state: IRateLimitState = {
        buckets: new Map(),
    ...
    async increment(key: string, config: IRateLimitConfig, incrementBy: number = 1): Promise<IRateLimitResult> {
    ...
    async getRemainingLimit(key: string, config: IRateLimitConfig): Promise<number> {
        await this.ensureInitialized();
    ...
    private getBucketsInWindow(key: string, now: number, windowMs: number, bucketSizeMs: number): IRateLimitBucket[] {
        const buckets: IRateLimitBucket[] = [];
    ...
    private async ensureInitialized(): Promise<void> {
        if (!this.initialized) {
            const stored = await this.ctx.storage.get<{
                buckets: [string, IRateLimitBucket][];
                lastCleanup: number;
            }>('state');
