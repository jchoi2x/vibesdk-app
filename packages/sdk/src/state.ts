...
function extractPhaseInfo(msg: unknown): {
  ...
  const phase = (
    msg as { phase?: { name?: string; description?: string } } | undefined
  )?.phase;
  return {
    name: phase?.name,
...
function extractPhaseFiles(
  ...
  const phase = (
    msg as
      | { phase?: { files?: { path: string; purpose: string }[] } }
      | undefined
  )?.phase;
  return phase?.files;
}

function isPhasicState(
  state: AgentState,
): state is AgentState & {
...
export class SessionStateStore {
    ...
    switch (msg.type) {
      ...
      case 'generation_complete': {
        ...
        const filesGenerated =
          'filesGenerated' in prev ? prev.filesGenerated : 0;
        this.setState({
          generation: {
            status: 'complete',
      ...
      case 'file_generating': {
        ...
        const phasesWithGenerating = this.updateFileStatus(
          m.filePath,
          'generating',
        );
        this.setState({
          currentFile: m.filePath,
          phases: phasesWithGenerating,
        });
        break;
      }
      case 'file_generated': {
      ...
      case 'phase_generating': {
        ...
        const phases = this.updateOrAddPhase(
          phaseInfo,
          'generating',
          phaseFiles,
        );
      ...
      case 'agent_connected': {
        ...
        const phases = buildPhaseTimelineFromState(
          agentState,
          agentState.generatedFilesMap ?? {},
        );

        // Determine generation state from agent state
        let generation = this.state.generation;
        if (agentState.shouldBeGenerating) {
          const filesGenerated = Object.keys(
            agentState.generatedFilesMap ?? {},
          ).length;
  ...
  private updateOrAddPhase(
    ...
    if (existingIndex >= 0) {
      ...
      phases[existingIndex] = {
        ...
        status,
        description:
          phaseInfo.description ?? phases[existingIndex]!.description,
  ...
  private emitPhaseChanges(
    prevPhases: PhaseInfo[],
    nextPhases: PhaseInfo[],
  ): void {
